...
Feature: Create a User External by API
  ...
  Scenario Outline: Create User External by API
    ...
    Examples:  
      ...
      | user-roles/00001-UserExternal-api-create/Missing_Parameter.json                | UserDto | /user/external |        400 | FAIL    | MISSING_PARAMETER                | The following parameters are required or contain invalid values: email.                              |
      | user-roles/00001-UserExternal-api-create/Missing_Parameter.json                | UserDto | /user/external |        400 | FAIL    | MISSING_PARAMETER                | The following parameters are required or contain invalid values: email.                              |
      | user-roles/00001-UserExternal-api-create/ENTITY_ALREADY_EXISTS_EXCEPTION.json  | UserDto | /user/external |        500 | FAIL    | ENTITY_ALREADY_EXISTS_EXCEPTION  | User with username=test5 already exists.                                                             |
