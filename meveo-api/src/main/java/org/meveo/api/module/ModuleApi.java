...
public class ModuleApi extends BaseApi {
	...
	private ModuleDto parseModule2Dto(MeveoModule meveoModule, ModuleDto moduleDto, User currentUser)
		...
		if (items != null && items.size() > 0) {
			...
			for (MeveoModuleItem item : items) {
				...
				case CET:
					CustomEntityTemplateDto cetDto = customEntityApi
							.findEntityTemplate(item.getItemCode(), currentUser);
					moduleDto.getCetDtos().add(cetDto);
					break;
				case CFT:
					CustomFieldTemplateDto cftDto = customFieldTemplateApi.find(item.getItemCode(),
							item.getAppliesTo(), provider);
				...
				case NOTIFICATION:
					...
					if (notification != null) {
						if (notification instanceof ScriptNotification) {
							...
							EmailNotificationDto emailNotifiDto = emailNotificationApi.find(item.getItemCode(),
									provider);
							...
							WebhookNotificationDto webhookNotifDto = webhookNotificationApi.find(item.getItemCode(),
									provider);
				...
				case MEASURABLEQUANTITIES:
					MeasurableQuantityDto measurableQuantityDto = measurableQuantityApi.find(item.getItemCode(),
							currentUser);
	...
	private ModuleDto getJobInstanceDto(String jobInstanceCode, User currentUser, ModuleDto moduleDto)
			throws MeveoApiException {
	...
	private MeveoModule parseModuleFromDto(MeveoModule meveoModule, ModuleDto moduleDto, User currentUser)
			throws MeveoApiException {
		meveoModule.setCode(moduleDto.getCode());
	...
	private void removeModulePicture(String provider, String filename) {
		try {
			...
			log.error("error when delete module picture {} for provider {}, info {}", filename, provider,
	...
	private byte[] readModulePicture(MeveoModule meveoModule, String filename) {
		try {
			...
			log.error("error when read module picture {}, info {}", filename, (e.getMessage() == null ? e.getClass()
