...
public class CatMessagesApi extends BaseApi {
	...
	public void create(CatMessagesDto postData, User currentUser)
		...
		if (!StringUtils.isBlank(postData.getObjectType())
			...
			if (tradingLanguageService.findByTradingLanguageCode(languageCode,
			...
			if (objectType.equals("Price plans")) {
				PricePlanMatrix pricePlanMatrix = pricePlanMatrixService
						.findByCode(objectTypeCode, currentUser.getProvider());
				if (pricePlanMatrix != null) {
					messageCode = pricePlanMatrix.getClass().getSimpleName()
							+ "_" + pricePlanMatrix.getId();
				} else {
					throw new EntityDoesNotExistsException(
							PricePlanMatrix.class, objectTypeCode);
				}
			}
			...
			if (objectType.equals("Charges")) {
				...
				if (usageChargeTemplate != null) {
					messageCode = usageChargeTemplate.getClass()
							.getSimpleName()
							+ "_"
				...
				if (recurringChargeTemplate != null) {
					messageCode = recurringChargeTemplate.getClass()
							.getSimpleName()
							+ "_"
				...
				if (oneShotChargeTemplate != null) {
					messageCode = oneShotChargeTemplate.getClass()
							.getSimpleName()
							+ "_"
				...
				if (messageCode == null) {
					throw new EntityDoesNotExistsException(
							ChargeTemplate.class, objectTypeCode);
				}
			}
			if (objectType.equals("Titles and civilities")) {
				Title title = titleService.findByCode(
						currentUser.getProvider(), objectTypeCode);
				if (title != null) {
					messageCode = title.getClass().getSimpleName() + "_"
							+ title.getId();
				} else {
			...
			if (objectType.equals("Taxes")) {
				Tax tax = taxService.findByCode(objectTypeCode,
						currentUser.getProvider());
				if (tax != null) {
					messageCode = tax.getClass().getSimpleName() + "_"
							+ tax.getId();
				} else {
			...
			if (objectType.equals("Invoice categories")) {
				...
				if (invoiceCategory != null) {
					messageCode = invoiceCategory.getClass().getSimpleName()
							+ "_" + invoiceCategory.getId();
				} else {
					throw new EntityDoesNotExistsException(
							InvoiceCategory.class, objectTypeCode);
				}
			}
			if (objectType.equals("Invoice subcategories")) {
				...
				if (invoiceSubCategory != null) {
					messageCode = invoiceSubCategory.getClass().getSimpleName()
							+ "_" + invoiceSubCategory.getId();
				} else {
					throw new EntityDoesNotExistsException(
							InvoiceSubCategory.class, objectTypeCode);
				}
			}

			if (messageCode != null) {
				CatMessages existingEntity = catMessagesService
						.findByCodeAndLanguage(messageCode, languageCode,
								currentUser.getProvider());
				if (existingEntity != null) {
					throw new EntityAlreadyExistsException(CatMessages.class,
							messageCode);
				} else {
	 ...
	 * @throws MeveoApiException
	 */
	public CatMessagesDto find(String catMessagesCode, String languageCode,
			Provider provider) throws MeveoApiException {

		CatMessagesDto catMessagesDto = null;

		if (!StringUtils.isBlank(catMessagesCode)
				&& !StringUtils.isBlank(languageCode)) {
			CatMessages catMessages = catMessagesService.findByCodeAndLanguage(
					catMessagesCode, languageCode, provider);

			if (catMessages == null) {
				throw new EntityDoesNotExistsException(CatMessages.class,
						catMessagesCode);
			}
			...
			catMessagesDto.setBasicDescription(catMessages
					.getEntityDescription());
			catMessagesDto.setDescriptionTranslation(catMessages
					.getDescription());
	...
	public void update(CatMessagesDto postData, User currentUser)
			throws MeveoApiException {
		if (!StringUtils.isBlank(postData.getObjectType())
				&& !StringUtils.isBlank(postData.getLanguageCode())
				&& !StringUtils.isBlank(postData.getEntityCode())
				&& !StringUtils.isBlank(postData.getDescriptionTranslation())
				&& !StringUtils.isBlank(postData.getCatMessagesCode())) {

			CatMessages catMessages = catMessagesService.findByCodeAndLanguage(
					postData.getCatMessagesCode(), postData.getLanguageCode(),
					currentUser.getProvider());

			if (catMessages == null) {
				throw new EntityDoesNotExistsException(CatMessages.class,
						postData.getCatMessagesCode());
			}
	...
	public void remove(String catMessagesCode, String languageCode,
			Provider provider) throws MeveoApiException {
	...
	public void createOrUpdate(CatMessagesDto postData, User currentUser)
			throws MeveoApiException, BusinessException {
