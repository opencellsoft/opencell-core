...
public class CustomerInvoiceApi extends BaseApi {
	...
	public List<CustomerInvoiceDto> getInvoiceList (String customerAccountCode,String providerCode) throws Exception {
		
		  
		List<CustomerInvoiceDto> customerInvoiceDtos = new ArrayList<CustomerInvoiceDto>(); 
		 
		if (!StringUtils.isBlank(customerAccountCode) && !StringUtils.isBlank(providerCode)) {	
			...
			CustomerAccount customerAccount = customerAccountService
				...
				for(BillingAccount billingAccount:customerAccount.getBillingAccounts()){
					...
					for (Invoice invoices : invoiceList) {
						...
						customerInvoiceDto.setBillingAccount(billingAccount.getCode()); 
						customerInvoiceDto.setInvoiceDate(invoices.getInvoiceDate());
						customerInvoiceDto.setDueDate(invoices.getDueDate());
						customerInvoiceDto.setAmount(invoices.getAmount());
						customerInvoiceDto.setAmount(invoices.getAmount());
						customerInvoiceDto.setAmountWithoutTax(invoices.getAmountWithoutTax());
						customerInvoiceDto.setInvoiceNumber(invoices.getInvoiceNumber());
						customerInvoiceDto.setProductDate(invoices.getProductDate());
						customerInvoiceDto.setNetToPay(invoices.getNetToPay());
						customerInvoiceDto.setPaymentMethod(invoices.getPaymentMethod().toString()!=null?invoices.getPaymentMethod().toString():null);
						...
						customerInvoiceDto.setInvoiceType(invoices.getInvoiceType()); 
						SubCategoryInvoiceAgregateDto subCategoryInvoiceAgregateDto = new SubCategoryInvoiceAgregateDto();
	
						for(InvoiceAgregate invoiceAgregate :invoices.getInvoiceAgregates()){
						    ...
						    customerInvoiceDto.addSubCategoryInvoiceAgregates(subCategoryInvoiceAgregateDto);
						}
