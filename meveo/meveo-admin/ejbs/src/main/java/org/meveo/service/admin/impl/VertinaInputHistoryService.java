...
public class VertinaInputHistoryService extends
	...
	public List<ChargeApplication> getChargeApplications(Long inputHistoryId,
			ApplicationChgStatusEnum status) {
		if (status == null) {
			Query q = em
					.createQuery("select ca from ChargeApplication ca where ca.inputHistoryId = :id");
	...
	@SuppressWarnings("unchecked")
	public List<ChargeApplication> getTransactions(Long inputHistoryId,
			RatedTransactionStatusEnum status) {
		if (status == null) {
			Query q = em
					.createQuery("select rt from RatedTransaction rt where rt.inputHistoryId = :id");
			q.setParameter("id", inputHistoryId);
			return q.getResultList();
		} else {
			Query q = em
