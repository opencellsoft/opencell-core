...
public class BillingAccountService extends AccountService<BillingAccount>
	...
	public void updateElectronicBilling(String code, Boolean electronicBilling,
	...
	public void updateBillingAccountDiscount(String code,
	...
	public BillingAccount billingAccountDetails(String code)
			throws BusinessException {
		BillingAccount billingAccount = findByCode(code);
	...
	public void billingAccountCancellation(String code, Date terminationDate,
			User updater) throws BusinessException {
		if (terminationDate == null) {
			terminationDate = new Date();
		}
		...
		List<UserAccount> userAccounts = billingAccount.getUsersAccounts();
		for (UserAccount userAccount : userAccounts) {
			userAccountService.userAccountCancellation(userAccount.getCode(),
					terminationDate, updater);
		}
		billingAccount.setTerminationDate(terminationDate);
	...
	public void billingAccountReactivation(String code, Date activationDate,
			User updater) throws BusinessException {
	...
	public void closeBillingAccount(String code, User updater)
	...
	public List<Invoice> invoiceList(String code) throws BusinessException {
		BillingAccount billingAccount = findByCode(code);
		if (billingAccount == null) {
			throw new BusinessException("Cannot found BillingAccount by code:"
					+ code);
		}
	...
	public Invoice InvoiceDetail(String invoiceReference) {
		try {
			...
			log.debug("invoice search returns no result for reference={0}.",
					invoiceReference);
		}
		return null;
	}
