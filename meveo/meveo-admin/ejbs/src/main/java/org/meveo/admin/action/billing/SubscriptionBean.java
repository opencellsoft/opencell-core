...
public class SubscriptionBean extends BaseBean<Subscription> {
	...
	public Subscription init() {
		...
		if (userAccountId != null) {
			UserAccount userAccount = userAccountService.findById(userAccountId.get());
	...
	public String saveOrUpdate() {
		...
		saveOrUpdate(entity);
		return "/pages/billing/subscriptions/subscriptionDetail?edit=false&objectId="
			...
			if (oneShotChargeInstance != null && oneShotChargeInstance.getId() != null) {
				...
				Long id = oneShotChargeInstanceService.oneShotChargeApplication(entity,
						...
						oneShotChargeInstance.getChargeDate() == null ? new Date()
								...
								.getAmountWithoutTax(), oneShotChargeInstance
								.getPrAmountWithoutTax(), oneShotChargeInstanceQuantity,
		...
		try {
			if (recurringChargeInstance != null) {
				if (recurringChargeInstance.getId() != null) {
					log.info("update RecurringChargeIns #0, id:#1", recurringChargeInstance,
					...
					Long id = recurringChargeInstanceService.recurringChargeApplication(entity,
							...
							recurringChargeInstance.getPrAmountWithoutTax(), 1,
	...
	@Produces
	@Named("recurringChargeInstances")
		...
		try {
			...
			log.info(
					...
					newSubscriptionTerminationReason != null ? newSubscriptionTerminationReason
							.getId() : null, terminationDate, selectedServiceInstanceId,
