...
public class ChargeApplicationService extends BusinessService<ChargeApplication> {
    ...
    public void oneShotChargeApplication(Subscription subscription, OneShotChargeInstance chargeInstance, Integer quantity, Date applicationDate, User creator)
        ...
        log.debug("ChargeApplicationService.oneShotChargeApplication subscriptionCode=#0,quantity=#1," + "applicationDate=#2,chargeInstance.getId=#3", subscription.getId(),
        ...
        if (tax == null) {
            throw new IncorrectChargeTemplateException("tax is null for invoiceSubCategory code=" + invoiceSubCategory.getCode());
        }
        ChargeApplication chargeApplication = new ChargeApplication(chargeTemplate.getCode(), chargeInstance.getDescription(), subscription, chargeInstance,
            chargeTemplate.getCode(), ApplicationChgStatusEnum.WAITING, ApplicationTypeEnum.PUNCTUAL, applicationDate, chargeInstance.getAmountWithoutTax(),
            chargeInstance.getAmount2(), quantity, tax.getCode(), tax.getPercent(), null, null, invoiceSubCategory, null, null, null, null, chargeInstance.getCriteria1(),
    ...
    public void recurringChargeApplication(Subscription subscription, RecurringChargeInstance chargeInstance, Integer quantity, Date applicationDate, User creator)
            throws BusinessException {
        ...
        log.debug("ChargeApplicationService.recurringChargeApplication subscriptionCode=#0,quantity=#1," + "applicationDate=#2,chargeInstance.getId=#3", subscription.getId(),
            quantity, applicationDate, chargeInstance.getId());
        ChargeTemplate chargeTemplate = chargeInstance.getChargeTemplate();
        if (chargeTemplate == null) {
            throw new IncorrectChargeTemplateException("chargeTemplate is null for chargeInstance id=" + chargeInstance.getId() + ", code=" + chargeInstance.getCode());
    ...
    public void chargeSubscription(RecurringChargeInstance chargeInstance, User creator) throws BusinessException {
        ...
        log.debug("ChargeApplicationService.chargeSubscription subscriptionCode=#0,chargeCode=#1,quantity=#2," + "applicationDate=#3,chargeInstance.getId=#4", chargeInstance
            .getServiceInstance().getSubscription().getCode(), chargeInstance.getCode(), chargeInstance.getServiceInstance().getQuantity(), chargeInstance.getSubscriptionDate(),
        ...
        try {
            ...
            nextDurationDate = durationTermCalendar != null ? durationTermCalendar.nextCalendarDate(applicationDate) : null;
        ...
        if (recurringChargeTemplate.getApplyInAdvance()) {
            ...
            log.debug("chargeSubscription applicationDate=#0, nextapplicationDate=#1,previousapplicationDate=#2", applicationDate, nextapplicationDate, previousapplicationDate);
            ...
            if (!recurringChargeTemplate.getSubscriptionProrata()) {
                ...
                if (part2 > 0) {
                    ...
                    log.error("Error in calendar dates : nextapplicationDate=#0, previousapplicationDate=#1", nextapplicationDate, previousapplicationDate);
                }

                log.debug("chargeSubscription part1=#0, part2=#1, param1=#2", part1, part2, param1);
            }

            param2 = " " + sdf.format(applicationDate) + " au " + sdf.format(DateUtils.addDaysToDate(nextapplicationDate, -1));
            ...
            ChargeApplication chargeApplication = new ChargeApplication(chargeInstance.getCode(), chargeInstance.getDescription(), chargeInstance.getServiceInstance()
                .getSubscription(), chargeInstance, chargeInstance.getCode(), ApplicationChgStatusEnum.WAITING, ApplicationTypeEnum.PRORATA_SUBSCRIPTION, applicationDate,
                chargeInstance.getAmountWithoutTax(), chargeInstance.getAmount2(), chargeInstance.getServiceInstance().getQuantity(), tax.getCode(), tax.getPercent(), null,
                nextapplicationDate, recurringChargeTemplate.getInvoiceSubCategory(), param1, param2, param3, null, chargeInstance.getCriteria1(), chargeInstance.getCriteria2(),
    ...
    public void applyReimbursment(RecurringChargeInstance chargeInstance, User creator) throws BusinessException {
        ...
        log.debug("applyReimbursment subscriptionCode=#0,chargeCode=#1,quantity=#2," + "applicationDate=#3,chargeInstance.getId=#4,NextChargeDate=#5", chargeInstance
            .getServiceInstance().getSubscription().getCode(), chargeInstance.getCode(), chargeInstance.getServiceInstance().getQuantity(), chargeInstance.getSubscriptionDate(),
        ...
        Date previousapplicationDate = recurringChargeTemplate.getCalendar().previousCalendarDate(applicationDate);
        previousapplicationDate = DateUtils.parseDateWithPattern(previousapplicationDate, "dd/MM/yyyy");
        log.debug("applicationDate=#0, nextapplicationDate=#1,previousapplicationDate=#2", applicationDate, nextapplicationDate, previousapplicationDate);
        ...
        if (recurringChargeTemplate.getTerminationProrata()) {
            ...
            param2 = " " + resourceMessages.getString("str_tooPerceived") + " " + sdf.format(periodStart) + " / " + sdf.format(DateUtils.addDaysToDate(nextapplicationDate, -1));
            ...
            ChargeApplication chargeApplication = new ChargeApplication(chargeInstance.getCode(), chargeInstance.getDescription(), chargeInstance.getServiceInstance()
                .getSubscription(), chargeInstance, chargeInstance.getCode(), ApplicationChgStatusEnum.WAITING, ApplicationTypeEnum.PRORATA_TERMINATION, applicationDate,
                chargeInstance.getAmountWithoutTax(), chargeInstance.getAmount2(), chargeInstance.getServiceInstance().getQuantity(), tax.getCode(), tax.getPercent(), null,
                nextapplicationDate, invoiceSubCategory, param1, param2, param3, null, chargeInstance.getCriteria1(), chargeInstance.getCriteria2(), chargeInstance.getCriteria3());
        ...
        if (recurringChargeTemplate.getApplyInAdvance()) {
            ...
            log.debug("reimbursment-applyInAdvance applicationDate=#0, nextapplicationDate=#1,nextChargeDate=#2", applicationDate, nextapplicationDate, nextChargeDate);
    ...
    public void applyReccuringCharge(RecurringChargeInstance chargeInstance, boolean reimbursement, RecurringChargeTemplate recurringChargeTemplate, User creator)
        ...
        String str_tooPerceived = resourceMessages.getString("str_tooPerceived");
        ...
        try {
            durationTermCalendar = serviceTemplate.getDurationTermCalendar();
            nextDurationDate = reimbursement ? chargeInstance.getNextChargeDate() : durationTermCalendar.nextCalendarDate(applicationDate);
            log.debug("reimbursement=#0,nextDurationDate=#1,applicationDate=#2", reimbursement, nextDurationDate, applicationDate);
        } catch (Exception e) {
        ...
        while (applicationDate.getTime() < nextDurationDate.getTime()) {
            Date nextapplicationDate = recurringChargeTemplate.getCalendar().nextCalendarDate(applicationDate);
            log.debug("next step for #0, applicationDate=#1, nextApplicationDate=#2,nextApplicationDate=#3", chargeInstance.getId(), applicationDate, nextapplicationDate,
                nextDurationDate);
            ...
            ChargeApplication chargeApplication = new ChargeApplication(chargeInstance.getCode(), chargeInstance.getDescription(), chargeInstance.getServiceInstance()
                .getSubscription(), chargeInstance, chargeInstance.getCode(), ApplicationChgStatusEnum.WAITING, reimbursement ? ApplicationTypeEnum.PRORATA_TERMINATION
                    : ApplicationTypeEnum.RECURRENT, applicationDate, chargeInstance.getAmountWithoutTax(), chargeInstance.getAmount2(), chargeInstance.getServiceInstance()
                .getQuantity(), tax.getCode(), tax.getPercent(), null, nextapplicationDate, invoiceSubCategory, reimbursement ? "-1" : null, param2, null, null,
    ...
    public void applyNotAppliedinAdvanceReccuringCharge(RecurringChargeInstance chargeInstance, boolean reimbursement, RecurringChargeTemplate recurringChargeTemplate, User creator)
            throws BusinessException {
        ...
        try {
            durationTermCalendar = serviceTemplate.getDurationTermCalendar();
            log.debug(" applyNotAppliedinAdvanceReccuringCharge nextChargeDate=#1,applicationDate=#2", nextChargeDate, applicationDate);
        } catch (Exception e) {
            log.error(" applyNotAppliedinAdvanceReccuringCharge Cannot find duration term calendar for serviceTemplate.id=#0", serviceTemplate.getId());
        }
        InvoiceSubCategory invoiceSubCategory = recurringChargeTemplate.getInvoiceSubCategory();
        if (invoiceSubCategory == null) {
            throw new IncorrectChargeTemplateException("invoiceSubCategory is null for chargeTemplate code=" + recurringChargeTemplate.getCode());
        ...
        while (applicationDate.getTime() < nextChargeDate.getTime()) {
            Date nextapplicationDate = recurringChargeTemplate.getCalendar().nextCalendarDate(applicationDate);
            log.debug("applyNotAppliedinAdvanceReccuringCharge next step for #0, applicationDate=#1, nextApplicationDate=#2,nextApplicationDate=#3", chargeInstance.getId(),
                applicationDate, nextapplicationDate, nextChargeDate);

            Date previousapplicationDate = recurringChargeTemplate.getCalendar().previousCalendarDate(applicationDate);
            previousapplicationDate = DateUtils.parseDateWithPattern(previousapplicationDate, "dd/MM/yyyy");
            log.debug(" applyNotAppliedinAdvanceReccuringCharge applicationDate=#0, nextapplicationDate=#1,previousapplicationDate=#2", applicationDate, nextapplicationDate,
            ...
            // charges �chues
            log.debug(" applyNotAppliedinAdvanceReccuringCharge chargeInstance.getChargeApplications().size()=#0", chargeInstance.getChargeApplications().size());
            if (chargeInstance.getChargeApplications().size() > 0 || !recurringChargeTemplate.getSubscriptionProrata()) {
                ...
                if (part2 > 0) {
                    ...
                    log.error("applyNotAppliedinAdvanceReccuringCharge Error in calendar dates : nextapplicationDate=#0, previousapplicationDate=#1", nextapplicationDate,
                        previousapplicationDate);
                }
                log.debug("part1=#0, part2=#1, param1=#2", part1, part2, param1);
            }

            param2 = (reimbursement ? str_tooPerceived + " " : " ") + sdf.format(applicationDate) + (reimbursement ? " / " : " au ")
                    + sdf.format(DateUtils.addDaysToDate(nextapplicationDate, -1));

            log.debug("param2=#0", param2);

            log.debug("applyNotAppliedinAdvanceReccuringCharge : nextapplicationDate=#0, param2=#1", nextapplicationDate, param2);

            ChargeApplication chargeApplication = new ChargeApplication(chargeInstance.getCode(), chargeInstance.getDescription(), chargeInstance.getServiceInstance()
                .getSubscription(), chargeInstance, chargeInstance.getCode(), ApplicationChgStatusEnum.WAITING, reimbursement ? ApplicationTypeEnum.PRORATA_TERMINATION
                    : applicationTypeEnum, applicationDate, chargeInstance.getAmountWithoutTax(), chargeInstance.getAmount2(), chargeInstance.getServiceInstance().getQuantity(),
                tax.getCode(), tax.getPercent(), null, nextapplicationDate, invoiceSubCategory, reimbursement ? "-1" : param1, param2, param3, null, chargeInstance.getCriteria1(),
                chargeInstance.getCriteria2(), chargeInstance.getCriteria3());
            chargeApplication.setSubscriptionDate(chargeInstance.getServiceInstance().getSubscriptionDate());
    ...
    public void applyChargeAgreement(RecurringChargeInstance chargeInstance, RecurringChargeTemplate recurringChargeTemplate, User creator) throws BusinessException {
        ...
        while (applicationDate.getTime() < endAgreementDate.getTime()) {
            Date nextapplicationDate = recurringChargeTemplate.getCalendar().nextCalendarDate(applicationDate);
            log.debug("agreement next step for #0, applicationDate=#1, nextApplicationDate=#2", recurringChargeTemplate.getCode(), applicationDate, nextapplicationDate);
            String param1 = null;
            ...
            ChargeApplication chargeApplication = new ChargeApplication(chargeInstance.getCode(), chargeInstance.getDescription(), chargeInstance.getServiceInstance()
                .getSubscription(), chargeInstance, chargeInstance.getCode(), ApplicationChgStatusEnum.WAITING, type, applicationDate, chargeInstance.getAmountWithoutTax(),
                chargeInstance.getAmount2(), chargeInstance.getServiceInstance().getQuantity(), tax.getCode(), tax.getPercent(), null, nextapplicationDate, invoiceSubCategory,
                param1, param2, null, null, chargeInstance.getCriteria1(), chargeInstance.getCriteria2(), chargeInstance.getCriteria3());
    ...
    public void cancelChargeApplications(Long chargeInstanceId, ChargeApplicationModeEnum mode, User updater) throws BusinessException {
        log.info("cancelChargeApplications chargeInstanceId=#0 ,mode=#1", chargeInstanceId, mode);
        List<ChargeApplication> chargeApplications = em.createQuery("from ChargeApplication where chargeInstance.id=:id and applicationMode=:mode")
    ...
    public void cancelOneShotChargeApplications(OneShotChargeInstance chargeInstance, OneShotChargeTemplateTypeEnum templateTypeEnum, User updater) throws BusinessException {
        log.info("cancelOneShotChargeApplications chargeInstanceId=#0 ,templateTypeEnum=#1", chargeInstance.getId(), templateTypeEnum);
        OneShotChargeTemplate oneShotChargeTemplate = null;
    ...
    public void cancelChgApplicationsAndTransactions(ChargeApplication chargeApplication, User updater) throws BusinessException {
    ...
    public void chargeTermination(RecurringChargeInstance chargeInstance, User creator) throws BusinessException {
        ...
        log.debug("ChargeApplicationService.chargeTermination subscriptionCode=#0,chargeCode=#1,quantity=#2," + "applicationDate=#3,chargeInstance.getId=#4", chargeInstance
            .getServiceInstance().getSubscription().getCode(), chargeInstance.getCode(), chargeInstance.getServiceInstance().getQuantity(), chargeInstance.getSubscriptionDate(),
            chargeInstance.getId());
        ...
        if (recurringChargeTemplate.getTerminationProrata()) {
            ...
            param2 = " " + str_tooPerceived + " " + sdf.format(periodStart) + " / " + sdf.format(DateUtils.addDaysToDate(nextapplicationDate, -1));
            log.debug("part1=#0, part2=#1, param1=#2, param2=#3", part1, part2, param1, param2);
