...
import org.meveo.model.users.Role;
...
public class MeveoPermissionResolver implements Serializable, PermissionResolver {
     ...
     * 
     * @see org.jboss.seam.security.permission.PermissionResolver#hasPermission(java.lang.Object, java.lang.String)
     */
    @Override
    public boolean hasPermission(Object resource, String permission) {
        ...
        if (!cachedPermissions.containsKey(cacheKey)) {
            ...
            boolean has = false;
            if (((MeveoUser) identity.getUser()) != null && ((MeveoUser) identity.getUser()).getUser() != null && ((MeveoUser) identity.getUser()).getUser().getRoles() != null) {
        ...
        // log.error("AKK check has permission " + cacheKey + " " + cachedPermissions.get(cacheKey));
