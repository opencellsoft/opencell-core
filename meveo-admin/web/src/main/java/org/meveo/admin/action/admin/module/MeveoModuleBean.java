...
public class MeveoModuleBean extends BaseBean<MeveoModule> {
	...
	public void init() {
		...
		measurableQuantityNode = new DefaultTreeNode(
				new CustomizedModuleItem("meveoModule.measurableQuantities", true), root);
	...
	public MeveoModule initEntity() {
		...
		if (module.getModuleItems() != null) {
			...
			while (itr.hasNext()) {
				...
				case CET:
					CustomEntityTemplate cet = customEntityTemplateService.findByCode(item.getItemCode(),
							getCurrentProvider());
					if (cet != null) {
						new DefaultTreeNode(new CustomizedModuleItem(cet.getCode(), cet.getDescription(),
								ModuleItemTypeEnum.CET), cetnode);
					} else {
					...
					break;
				case CFT:
					CustomFieldTemplate cft = customFieldTemplateService.findByCodeAndAppliesTo(item.getItemCode(),
							item.getAppliesTo(), getCurrentProvider());
					if (cft != null) {
						new DefaultTreeNode(new CustomizedModuleItem(cft.getCode(), cft.getDescription(),
								cft.getAppliesTo(), ModuleItemTypeEnum.CFT), cftnode);
					} else {
						item.setMeveoModule(null);
						itr.remove();
					}
				...
				case FILTER:
					...
					if (filter != null) {
						new DefaultTreeNode(new CustomizedModuleItem(filter.getCode(), filter.getDescription(),
								ModuleItemTypeEnum.FILTER), filternode);
					} else {
						item.setMeveoModule(null);
						itr.remove();
					}
				...
				case SCRIPT:
					...
					if (script != null) {
						new DefaultTreeNode(new CustomizedModuleItem(script.getCode(), script.getDescription(),
								ModuleItemTypeEnum.SCRIPT), scriptnode);
					} else {
						item.setMeveoModule(null);
						itr.remove();
					}
				...
				case JOBINSTANCE:
					...
					if (job != null) {
						new DefaultTreeNode(new CustomizedModuleItem(job.getCode(), job.getDescription(),
								ModuleItemTypeEnum.JOBINSTANCE), jobnode);
					} else {
						item.setMeveoModule(null);
						itr.remove();
					}
				...
				case NOTIFICATION:
					Notification notification = notificationService
							.findByCode(item.getItemCode(), getCurrentProvider());
					if (notification != null) {
						new DefaultTreeNode(new CustomizedModuleItem(notification.getCode(),
								notification.getDescription(), ModuleItemTypeEnum.NOTIFICATION), notificationnode);
					} else {
						item.setMeveoModule(null);
						itr.remove();
					}
				...
				case MEASURABLEQUANTITIES:
					...
					if (measurableQuantity != null) {
						new DefaultTreeNode(new CustomizedModuleItem(measurableQuantity.getCode(),
								measurableQuantity.getDescription(), ModuleItemTypeEnum.MEASURABLEQUANTITIES),
				...
				case CHART:
					...
					if (chart != null) {
						new DefaultTreeNode(new CustomizedModuleItem(chart.getCode(), chart.getDescription(),
								ModuleItemTypeEnum.CHART), chartNode);
					} else {
						item.setMeveoModule(null);
						itr.remove();
					}
	...
	public void setCustomEntity(CustomEntityTemplate customEntity) {
		...
		if (customEntity != null) {
			...
			if (!entity.getModuleItems().contains(item)) {
				...
				new DefaultTreeNode(new CustomizedModuleItem(customEntity.getCode(), customEntity.getDescription(),
						ModuleItemTypeEnum.CET), cetnode);
			}
	...
	public void setCustomField(CustomFieldTemplate customField) {
		if (customField != null) {
			MeveoModuleItem item = new MeveoModuleItem(customField.getCode(), customField.getAppliesTo(),
					ModuleItemTypeEnum.CFT);
			if (!entity.getModuleItems().contains(item)) {
				entity.addModuleItem(item);
				new DefaultTreeNode(new CustomizedModuleItem(customField.getCode(), customField.getDescription(),
						customField.getAppliesTo(), ModuleItemTypeEnum.CFT), cftnode);
			}
		}
	}
	...
	public void setNotification(Notification notification) {
		if (notification != null) {
			...
			if (!entity.getModuleItems().contains(item)) {
				entity.addModuleItem(item);
				new DefaultTreeNode(new CustomizedModuleItem(notification.getCode(), notification.getDescription(),
						ModuleItemTypeEnum.NOTIFICATION), notificationnode);
			}
		}
	}
	...
	public void exportModule() {
		...
		if (meveoInstance != null) {
			try {
				meveoModuleService.exportModule2MeveoInstance(entity, meveoInstance, this.currentUser);
				messages.info(new BundleKey("messages", "meveoModule.exportSuccess"), entity.getCode(),
						meveoInstance.getCode());
			} catch (Exception e) {
				log.error("Error when export module {} to {}", entity.getCode(), meveoInstance, e);
				messages.error(new BundleKey("messages", "meveoModule.exportFailed"), entity.getCode(),
						meveoInstance.getCode(),
	...
	public void cropLogo() {
		try {
			...
			log.error("error when crop a module picture {}, info {}!", croppedImage.getOriginalFilename(),
					e.getMessage());
			messages.error(new BundleKey("messages", "meveoModule.cropPictureFailed"), (e.getMessage() == null ? e
	...
	public void handleFileUpload(FileUploadEvent event) {
		...
		if (!"JPEG".equalsIgnoreCase(formatname) && !"JPG".equalsIgnoreCase(formatname)
				&& !"PNG".equalsIgnoreCase(formatname) && !"GIF".equalsIgnoreCase(formatname)) {
		...
		try {
			...
			log.error("Failed to upload a picture {} for module {}, info {}", filename, entity.getCode(),
					e.getMessage(), e);
			messages.error(new BundleKey("messages", "meveoModule.uploadPictureFailed"), (e.getMessage() == null ? e
					.getClass().getSimpleName() : e.getMessage()));
		} finally {
	...
	public void setMeveoModule(MeveoModule meveoModule) {
		if (meveoModule != null) {
			...
			if (!entity.getModuleItems().contains(item)) {
				entity.addModuleItem(item);
				new DefaultTreeNode(new CustomizedModuleItem(meveoModule.getCode(), meveoModule.getDescription(),
						ModuleItemTypeEnum.SUBMODULE), subModuleNode);
			}
		}
	}
	...
	public void setMeasurableQuantity(MeasurableQuantity measurableQuantity) {
		if (measurableQuantity != null) {
			MeveoModuleItem item = new MeveoModuleItem(measurableQuantity.getCode(),
					ModuleItemTypeEnum.MEASURABLEQUANTITIES);
			if (!entity.getModuleItems().contains(item)) {
				entity.addModuleItem(item);
				new DefaultTreeNode(new CustomizedModuleItem(measurableQuantity.getCode(),
						measurableQuantity.getDescription(), ModuleItemTypeEnum.MEASURABLEQUANTITIES),
