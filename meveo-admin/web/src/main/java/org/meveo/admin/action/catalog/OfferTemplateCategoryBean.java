...
public class OfferTemplateCategoryBean extends CustomFieldBean<OfferTemplateCategory> {
	...
	public void onDragDrop(TreeDragDropEvent event) {
		...
		if (dropNode != null) {
			...
			try {
				// UnSelected previous selected node to make sure one node
				// selected after drag drop
	...
	private void saveOrUpdateImport(OfferTemplateCategory iCat) throws BusinessException {
		...
		// find
		OfferTemplateCategory cat = offerTemplateCategoryService.findByCode(iCat.getCode(), getCurrentProvider());
		if (cat != null) {
			// update
			if (!StringUtils.isBlank(iCat.getParentCategoryCode())) {
				if (cat.getOfferTemplateCategory() == null
					...
					// update parent
					OfferTemplateCategory parent = offerTemplateCategoryService.findByCode(iCat.getParentCategoryCode(), getCurrentProvider());
					if (parent != null) {
			...
			getPersistenceService().update(cat, getCurrentUser());
			...
			// create
			getPersistenceService().create(iCat, getCurrentUser());
