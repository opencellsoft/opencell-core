...
public class PDFFilesOutputProducer {
	...
	private Logger logger = Logger.getLogger(PDFFilesOutputProducer.class
			.getName());
	...
	public void producePdf(Map<String, Object> parameters) throws Exception {
		...
		logger.info("PDFInvoiceGenerationJob is invoice key exists="
		...
		String pdfDirectory = meveoDir + "invoices" + File.separator + "pdf"
				+ File.separator;

		(new File(pdfDirectory)).mkdirs();
		String INVOICE_TAG_NAME = "invoice";

		File billingRundir = new File(meveoDir + "invoices" + File.separator
				+ "xml" + File.separator + invoice.getBillingRun().getId());
		...
		logger.info(String.format("Jasper template used: %s",
		...
		Node invoiceNode = xmlDocument.getElementsByTagName(INVOICE_TAG_NAME)
				.item(0);
