...
/**
 * Provides cache related services (loading, update) for rating related
 * operations
 * 
...
public class RatingCacheContainerProvider {
	...
	/**
	 * Contains association between charge code and price plans. Key format:
	 * <provider id>_<charge template code, which is pricePlanMatrix.eventCode>
	 */
	...
	/**
	 * Contains association between usage charge template id and cached usage
	 * charge template information. Key format: UsageChargeTemplate.id
	 */
	// @Resource(lookup =
	...
	/**
	 * Contains association between subscription and usage charge instances. Key
	...
	/**
	 * Contains association between counter instance id and cached counter
	 * information. Key format: CounterInstance.id
	 */
	// @Resource(lookup =
	...
	public void removePricePlanFromCache(PricePlanMatrix pricePlan) {
		...
		for (PricePlanMatrix chargePricePlan : chargePriceList) {
			if (pricePlan.getId().equals(chargePricePlan.getId())) {
				...
				log.info("Removed pricePlan from cache for provider=" + pricePlan.getProvider().getCode()
						+ "; chargeCode=" + pricePlan.getEventCode() + "; priceplan=" + pricePlan);
				break;
			}
			index++;
		}
	...
	private void populateUsageChargeCache() {
		...
		List<UsageChargeInstance> usageChargeInstances = usageChargeInstanceService
				.getAllUsageChargeInstancesForCache();
	...
	public void updateUsageChargeInstanceInCache(UsageChargeInstance usageChargeInstance) {
		...
		UsageChargeTemplate usageChargeTemplate = usageChargeTemplateService.findById(usageChargeInstance
				.getChargeTemplate().getId());
		Long subscriptionId = usageChargeInstance.getServiceInstance().getSubscription().getId();
		log.info(
				"Updating usageChargeInstance cache with usageChargeInstance: subscription Id: {}, charge id={}, usageChargeTemplate id: {}",
				subscriptionId, usageChargeInstance.getId(), usageChargeTemplate.getId());
			...
			for (UsageChargeInstanceCache charge : charges) {
				if (charge.getChargeInstanceId() == usageChargeInstance.getId()) {
					if (usageChargeInstance.getStatus() != InstanceStatusEnum.ACTIVE) {
						log.info(
								"The cache contains the UsageChargeInstance {} but its status in db is not active so we remove it",
		...
		log.info("UsageChargeInstance " + (!cacheContainsCharge ? "added" : "updated")
				+ " in usageChargeInstanceCache: subscription id {}, charge id {}", subscriptionId,
				usageChargeInstance.getId());
	 ...
	 * 
	 * @param usageChargeTemplate
	 *            Usage charge template to update
	...
	public UsageChargeTemplateCache updateUsageChargeTemplateInCache(UsageChargeTemplate usageChargeTemplate) {
		...
		log.info("UsageChargeTemplate " + (addedToCache ? "added" : "updated")
				+ " in usageChargeTemplateCacheCache: template {}", usageChargeTemplate.getCode());

		return cachedTemplate;
	}

	/**
	 * Update cache with usage charge templates that are associated to triggered
	 * EDR template
	 * 
	 * @param triggeredEDRTemplate
	 *            Triggered EDR template
	 */
	public void updateUsageChargeTemplateInCache(TriggeredEDRTemplate triggeredEDRTemplate) {
		...
		List<UsageChargeTemplate> charges = usageChargeTemplateService
				.findAssociatedToEDRTemplate(triggeredEDRTemplate);
	...
	public void restoreCounters(Map<Long, BigDecimal> counterPeriodValues) throws BusinessException {
		for (Long cId : counterPeriodValues.keySet()) {
			...
			if (counterCacheValue != null) {
				for (CounterPeriodCache cpc : counterCacheValue.getCounterPeriods()) {
					if (cpc.getCounterPeriodId() == cId) {
						...
						log.debug("Added {} to counter period in cache (new value {}, period id {})", value,
								cpc.getValue(), cId);
						break;
					}
				}
			...
			log.debug("Added {}  (new value {}) to counterPeriod {}, counter {}", value, counterPeriod.getValue(),
					counterPeriod.getId(), counterInstanceId);
		}
	}
