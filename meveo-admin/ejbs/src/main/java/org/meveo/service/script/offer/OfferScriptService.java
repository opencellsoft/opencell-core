...
public class OfferScriptService extends CustomScriptService<OfferModelScript, OfferScriptInterface> {
    ...
    public void subscribe(IEntity entity, String scriptCode, User currentUser) throws InvalidPermissionException, ElementNotFoundException, BusinessException {
    ...
    public void terminate(IEntity entity, String scriptCode, Map<String, Object> scriptContext, User currentUser) throws InvalidPermissionException, ElementNotFoundException,
            BusinessException {
        OfferScriptInterface scriptInterface = getScriptInstance(currentUser.getProvider(), scriptCode);
        if (scriptInterface != null) {
            scriptContext.put(Script.CONTEXT_ENTITY, entity);
            scriptInterface.terminate(scriptContext, currentUser.getProvider(), currentUser);
        }
    }

    public void create(IEntity entity, String scriptCode, User currentUser) throws InvalidPermissionException, ElementNotFoundException, BusinessException {
        OfferScriptInterface scriptInterface = getScriptInstance(currentUser.getProvider(), scriptCode);
        if (scriptInterface != null) {
            Map<String, Object> scriptContext = new HashMap<String, Object>();
            scriptContext.put(Script.CONTEXT_ENTITY, entity);
            scriptInterface.create(scriptContext, currentUser.getProvider(), currentUser);
        }
    }
