...
public class AccountHierarchyApiService extends BaseApi {
	...
	public void create(AccountHierarchyDto postData, User currentUser) throws MeveoApiException,
		...
		if (customerService.findByCode(postData.getCustomerId(), provider) != null) {
			...
			if (!StringUtils.isBlank(postData.getCustomerId()) && !StringUtils.isBlank(postData.getCustomerBrandCode())
				...
				String cfInstancePaymentMethodCode = "api.default.customerAccount.paymentMethod";
				CustomFieldInstance cfInstancePaymentMethond = (CustomFieldInstance) providerService
						.getCustomFieldOrProperty(cfInstancePaymentMethodCode, "1", provider, true,
								AccountLevelEnum.PROVIDER, currentUser);

				if (cfInstancePaymentMethond == null) {
					throw new EntityDoesNotExistsException("Custom Field Instance with code "
							+ cfInstancePaymentMethodCode + " does not exist");
				}

				String cfInstanceCreditCategoryCode = "api.default.customerAccount.creditCategory";
				CustomFieldInstance cfInstanceCreditCategory = (CustomFieldInstance) providerService
						.getCustomFieldOrProperty(cfInstanceCreditCategoryCode, "NEWCUSTOMER", provider, true,
								AccountLevelEnum.PROVIDER, currentUser);

				if (cfInstanceCreditCategory == null) {
					throw new EntityDoesNotExistsException("Custom Field Instance with code "
							+ cfInstanceCreditCategoryCode + " does not exist");
				}

				int caPaymentMethod = Integer.parseInt(cfInstancePaymentMethond.getValueAsString());
				String creditCategory = cfInstanceCreditCategory.getValueAsString();
				int baPaymentMethod = Integer.parseInt(cfInstancePaymentMethond.getValueAsString());
				...
				String cfElectronicBillingCode = "api.customerHeirarchy.billingAccount.electronicBilling";
				CustomFieldInstance cfInstance = (CustomFieldInstance) providerService.getCustomFieldOrProperty(
						cfElectronicBillingCode, "true", customerAccount.getProvider(), true,
						AccountLevelEnum.PROVIDER, currentUser);

				if (cfInstance == null) {
					throw new EntityDoesNotExistsException("Custom Field Instance with code " + cfElectronicBillingCode
							+ " does not exist.");
				}
				billingAccount.setElectronicBilling(Boolean.valueOf(cfInstance.getValueAsString()));
	...
	public void update(AccountHierarchyDto postData, User currentUser) throws MeveoApiException,
		...
		if (!StringUtils.isBlank(postData.getCustomerId()) && !StringUtils.isBlank(postData.getCustomerBrandCode())
			...
			if (cfInstancePaymentMethond == null) {
				throw new EntityDoesNotExistsException("Custom Field Instance with code " + cfInstancePaymentMethodCode
						+ " does not exist");
			}
			...
			CustomFieldInstance cfInstance = (CustomFieldInstance) providerService.getCustomFieldOrProperty(
					"api.customerHeirarchy.billingAccount.electronicBilling", "true", customerAccount.getProvider(),
					true, AccountLevelEnum.PROVIDER, currentUser);
			billingAccount.setElectronicBilling(Boolean.valueOf(cfInstance.getValueAsString()));
			billingAccount.setTradingCountry(tradingCountry);
			billingAccount.setTradingLanguage(tradingLanguage);
			billingAccount.setBillingCycle(billingCycle);
