...
public class WalletOperationService extends BusinessService<WalletOperation> {
	...
	//cache the association between usage chargeInstance and wallets (if it is not the only principal one)
	...
	public void buildCacheFromDB(){
		//for each recrring usage charInstance of active subscription we create association	
		List<UsageChargeInstance> charges = (List<UsageChargeInstance>) getEntityManager()
				.createNamedQuery("UsageChargeInstance.listPrepaidActive", UsageChargeInstance.class).getResultList();
		for(UsageChargeInstance charge:charges){
			updateCache(charge);
		}
		List<Long> walletIds =  (List<Long>) getEntityManager()
				.createNamedQuery("WalletInstance.listPrepaidActiiveWalletIds", Long.class).getResultList();
	...
	public void fillBalanceCaches(Long walletId){
		BigDecimal balance = (BigDecimal) getEntityManager()
				.createNamedQuery("WalletOperation.getBalance", BigDecimal.class).setParameter("walletId", walletId).getSingleResult();
		if(balance==null){
			balance=BigDecimal.ZERO;
		}
		balanceCache.put(walletId,balance);
		BigDecimal reservedBalance = (BigDecimal) getEntityManager()
				.createNamedQuery("WalletOperation.getReservedBalance", BigDecimal.class).setParameter("walletId", walletId).getSingleResult();
	...
	/*public WalletOperation rateOneShotApplication(Subscription subscription, OneShotChargeInstance chargeInstance,
			Integer quantity, Date applicationDate) throws BusinessException {
		return rateOneShotApplication(getEntityManager(), subscription, chargeInstance, quantity, applicationDate,
				getCurrentUser());
	}*/
	...
	public WalletOperation rateSubscription(EntityManager em, Date subscriptionDate,
		...
		if (Boolean.TRUE.equals(recurringChargeTemplate.getSubscriptionProrata())) {
			...
			log.debug("rateSubscription part1={}, part2={}, prorataRation={} -> quantity={}", part1, part2,
	...
	public void applyNotAppliedinAdvanceReccuringCharge(EntityManager em, RecurringChargeInstance chargeInstance,
		...
		while (applicationDate.getTime() < nextChargeDate.getTime()) {
			...
			List<WalletOperation> oprations=chargeWalletOpertation(walletOperation, creator, chargeInstance.getProvider());
	...
	//charging
	public List<WalletOperation>  chargeOnWalletIds(List<Long> walletIds,WalletOperation op, User creator, Provider provider) throws BusinessException{
		...
		if(remainingAmountToCharge.compareTo(totalBalance)>0 && reservedBalanceCache.containsKey(walletIds.get(walletIds.size()-1))){
	...
	public List<WalletOperation> chargeWalletOpertation(WalletOperation op, User creator, Provider provider) throws BusinessException{
		List<WalletOperation> result =  new ArrayList<>();
		...
		if(usageChargeInstanceWallet.containsKey(op.getChargeInstance().getId())){
			...
			log.debug("chargeWalletOpertation chargeInstanceId found in usageCache with {} wallet ids",walletIds.size());
			...
			log.debug("chargeWalletOpertation is recurring or oneshot, and associated to {} wallet ids",walletIds.size());
			result =  chargeOnWalletIds(walletIds, op, creator, provider);
		} else if(!op.getChargeInstance().isPrepaid()){
