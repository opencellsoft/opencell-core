...
//		}
	...
	public void validate(Long billingRunId,User currentUser,long nbRuns,long waitingMillis) throws Exception{
		...
		Provider provider = billingRun.getProvider();
		...
		if (BillingRunStatusEnum.NEW.equals(billingRun.getStatus())) {
			...
			billingRun = billingRunExtensionService.updateBRAmounts(billingRun);
			...
			if (billingAccounts != null && billingAccounts.size() > 0) {
				...
				while (subListCreator.isHasNext()) {
					Future<Integer> count = invoicingAsync.updateBillingAccountTotalAmountsAsync((List<BillingAccount>) subListCreator.getNextWorkSet(), billingRun, currentUser);
				...
				billingRun = billingRunExtensionService.updateBillingRun(billingRun.getId(),currentUser,billingAccounts.size(),billableBA,BillingRunStatusEnum.PREINVOICED,new Date());
				billingRun = refreshOrRetrieve(billingRun);
				if (billingRun.getProcessType() == BillingProcessTypesEnum.AUTOMATIC || provider.isAutomaticInvoicing()) {
					createAgregatesAndInvoice(billingRun.getId(), billingRun.getLastTransactionDate(), currentUser, nbRuns, waitingMillis);
					billingRun = billingRunExtensionService.updateBillingRun(billingRun.getId(), currentUser, null, null, BillingRunStatusEnum.POSTINVOICED, null);
				}
			}
		} else if (BillingRunStatusEnum.PREVALIDATED.equals(billingRun.getStatus())) {
			createAgregatesAndInvoice(billingRun.getId(),billingRun.getLastTransactionDate(), currentUser,nbRuns,waitingMillis);								
			billingRunExtensionService.updateBillingRun(billingRun.getId(),currentUser,null,null,BillingRunStatusEnum.POSTINVOICED,null);
	...
	public void forceValidate(Long billingRunId,User currentUser) throws BusinessException{
		...
		case PREVALIDATED:
			...
			billingRunExtensionService.updateBillingRun(billingRun.getId(),currentUser,1,0,BillingRunStatusEnum.POSTINVOICED,null);
			break;
		case VALIDATED:
		case CANCELED:
		case NEW:
