...
public class PricePlanMatrixService extends PersistenceService<PricePlanMatrix> {
	...
	String[] colNames={"Charge code","Seller","Country","Currency","Start appli.","End appli.",
			"Offer code","Priority","Amount w/out tax","Amount with tax","Min quantity","Max quantity",
			"Criteria 1","Criteria 2","Criteria 3","Criteria EL","Start rating","End rating",
			"Min subscr age","Max subscr age"};
	
	@TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
	public int importFromExcel(EntityManager em,InputStream excelInputStream,User user,Provider provider) throws BusinessException{
		int result=0;
		Workbook workbook;
		...
		try {
			...
			for(int i=0;i<headerCells.length;i++){
				if(!colNames[i].equalsIgnoreCase(headerCells[i].getContents())){
					throw new BusinessException("Invalid column "+i+" found ["+headerCells[i].getContents()+"] but was expecting ["+colNames[i]+"]");
				}
			}
			for(int rowIndex=1;rowIndex<nbRows;rowIndex++){
				...
				if(pricePlan==null) {
					log.warn("there are several pricePlan records for charge "+cells[0].getContents()+" we do not update them");
				} else {
					...
					if(!StringUtils.isBlank(startSub)){
						try{
							...
							throw new BusinessException("Invalid startSub in line "+rowIndex+", startSub="+startSub+" expected format:"+param.getProperty("excelImport.dateFormat", "dd/MM/yyyy")+", you may change the property excelImport.dateFormat.");
						}
					} else {
					...
					if(!StringUtils.isBlank(endSub)){
						try{
							...
							throw new BusinessException("Invalid endSub in line "+rowIndex+", endSub="+endSub+" expected format:"+param.getProperty("excelImport.dateFormat", "dd/MM/yyyy")+", you may change the property excelImport.dateFormat.");
						}
					} else {
					...
					if(!StringUtils.isBlank(startRating)){
						try{
							...
							throw new BusinessException("Invalid startRating in line "+rowIndex+", startRating="+startRating+" expected format:"+param.getProperty("excelImport.dateFormat", "dd/MM/yyyy")+", you may change the property excelImport.dateFormat.");
						}
					} else {
					...
					if(!StringUtils.isBlank(endRating)){
						try{
							...
							throw new BusinessException("Invalid endRating in line "+rowIndex+", endRating="+endRating+" expected format:"+param.getProperty("excelImport.dateFormat", "dd/MM/yyyy")+", you may change the property excelImport.dateFormat.");
						}
					} else {
