...
public class WalletCacheContainerProvider implements Serializable { // CacheContainerProvider, Serializable {
    ...
    private void init() {
        try {
            ...
            log.debug("WalletCacheContainerProvider initializing for provider {}...", currentUserProvider.getCurrentUserProviderCode());
            ...
            log.info("WalletCacheContainerProvider initialized for provider {}", currentUserProvider.getCurrentUserProviderCode());
        } catch (Exception e) {
    ...
    private void populateWalletCache() {
        ...
        // FIXME @wassim need to clear only provider data 
        balanceCache.clear();
        ...
        log.debug("Start to pre-populate Prepaid balance cache for Provider {}", currentUserProvider.getCurrentUserProviderCode());
    ...
    public List<Long> addUsageChargeInstance(UsageChargeInstance usageChargeInstance) {
        ...
        for (WalletInstance wallet : wallets) {
            if (!walletIds.contains(wallet.getId()) && wallet.getWalletTemplate() != null && wallet.getWalletTemplate().getWalletType() == BillingWalletTypeEnum.PREPAID) {
                ...
                if (!balanceCache.containsKey(new CacheKeyLong(wallet.getId(), currentUserProvider.getCurrentUserProviderCode()))) {
        ...
        usageChargeInstanceWalletCache.getAdvancedCache().withFlags(Flag.IGNORE_RETURN_VALUES).put(new CacheKeyLong(usageChargeInstance.getId(), currentUserProvider.getCurrentUserProviderCode()), walletIds);
    ...
    public BigDecimal initializeBalanceCachesForWallet(Long walletId) {
        ...
        balanceCache.getAdvancedCache().withFlags(Flag.IGNORE_RETURN_VALUES).put(new CacheKeyLong(walletId, currentUserProvider.getCurrentUserProviderCode()), balance);

        BigDecimal reservedBalance = walletService.calculateWalletReservedBalance(walletId);
        if (reservedBalance == null) {
            reservedBalance = BigDecimal.ZERO;
        }
        reservedBalanceCache.getAdvancedCache().withFlags(Flag.IGNORE_RETURN_VALUES).put(new CacheKeyLong(walletId, currentUserProvider.getCurrentUserProviderCode()), reservedBalance);
    ...
    public void updateBalance(WalletOperation op) {
        ...
        if (!reservedBalanceCache.containsKey(new CacheKeyLong(walletId, currentUserProvider.getCurrentUserProviderCode())) 
        		|| !balanceCache.containsKey(new CacheKeyLong(walletId, currentUserProvider.getCurrentUserProviderCode()))) {
            initializeBalanceCachesForWallet(walletId);
            return;
        }
        ...
        if (!(op instanceof WalletReservation) || (op.getStatus() == WalletOperationStatusEnum.RESERVED) || (op.getStatus() == WalletOperationStatusEnum.CANCELED)) {
            ...
            oldValue = reservedBalanceCache.getAdvancedCache().withFlags(Flag.FORCE_WRITE_LOCK).get(new CacheKeyLong(op.getWallet().getId(), currentUserProvider.getCurrentUserProviderCode()));
            ...
            reservedBalanceCache.getAdvancedCache().withFlags(Flag.IGNORE_RETURN_VALUES).put(
            		new CacheKeyLong(currentUserProvider.getCurrentUserProviderCode(), op.getWallet().getId()), 
            		newValue
            		);
        }
        ...
        if (!(op instanceof WalletReservation) || (op.getStatus() == WalletOperationStatusEnum.OPEN)) {
            ...
            oldValue = balanceCache.getAdvancedCache().withFlags(Flag.FORCE_WRITE_LOCK).get(new CacheKeyLong(op.getWallet().getId(), currentUserProvider.getCurrentUserProviderCode()));
            newValue = oldValue.subtract(op.getAmountWithTax());
            ...
            balanceCache.getAdvancedCache().withFlags(Flag.IGNORE_RETURN_VALUES).put(new CacheKeyLong(currentUserProvider.getCurrentUserProviderCode(), op.getWallet().getId()), newValue);
    ...
    public BigDecimal getBalance(Long walletId) {
        BigDecimal result = balanceCache.get(new CacheKeyLong(walletId, currentUserProvider.getCurrentUserProviderCode()));
        return result;
    }
    ...
    public BigDecimal getReservedBalance(Long walletId) {
        BigDecimal result = reservedBalanceCache.get(new CacheKeyLong(walletId, currentUserProvider.getCurrentUserProviderCode()));
        return result;
    }
    ...
    public List<Long> getWalletIds(Long usageChargeInstanceId) {
        return usageChargeInstanceWalletCache.get(new CacheKeyLong(usageChargeInstanceId, currentUserProvider.getCurrentUserProviderCode()));
    }
