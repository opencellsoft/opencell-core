                            ...
                            if (!StringUtils.isBlank(serviceMinAmountEL)) {
                                ...
                                if (appProvider.isEntreprise()) {
                                    ...
                                    if (totalServiceAmountWithoutTax.compareTo(BigDecimal.ZERO) != 0) {
                                        ratio = serviceAmountWithoutTax.divide(totalServiceAmountWithoutTax, appProvider.getRounding(), RoundingMode.HALF_UP);
                                    } else {
                                    ...
                                    if (totalServiceAmountWithTax.compareTo(BigDecimal.ZERO) != 0) {
                                        ratio = serviceAmountWithTax.divide(totalServiceAmountWithTax, appProvider.getRounding(), RoundingMode.HALF_UP);
                                    } else {
                                        ratio = BigDecimal.ONE;
                                    }
                                }
                                ...
                                if (diff.doubleValue() > 0) {
                                    ...
                                    BigDecimal unitAmountWithoutTax = appProvider.isEntreprise() ? rtMinAmount
                                            : rtMinAmount.subtract(rtMinAmount.multiply(taxPercent).divide(new BigDecimal(100), appProvider.getRounding(), RoundingMode.HALF_UP));
                                    BigDecimal unitAmountWithTax = appProvider.isEntreprise()
                                            ? rtMinAmount.add(rtMinAmount.multiply(taxPercent).divide(new BigDecimal(100), appProvider.getRounding(), RoundingMode.HALF_UP))
                                            : rtMinAmount;
                    ...
                    if (!StringUtils.isBlank(subscriptionMinAmountEL)) {
                        ...
                        if (appProvider.isEntreprise()) {
                            ...
                            if (totalSubscriptionAmountWithoutTax.compareTo(BigDecimal.ZERO) != 0) {
                                ratio = subscriptionAmountWithoutTax.divide(totalSubscriptionAmountWithoutTax, appProvider.getRounding(), RoundingMode.HALF_UP);
                            } else {
                                ratio = BigDecimal.ONE;
                            }
                        } else {
                            ...
                            if (totalSubscriptionAmountWithTax.compareTo(BigDecimal.ZERO) != 0) {
                                ratio = subscriptionAmountWithTax.divide(totalSubscriptionAmountWithTax, appProvider.getRounding(), RoundingMode.HALF_UP);
                            } else {
                                ratio = BigDecimal.ONE;
                            }
                        }
                ...
                if (!StringUtils.isBlank(billingAccount.getMinimumAmountEl()) && billingAccountAmountWithoutTax != null && billingAccountAmountWithoutTax != BigDecimal.ZERO) {
                    ...
                    if (appProvider.isEntreprise()) {
                        ...
                        if (totalBillingAccountAmountWithoutTax.compareTo(BigDecimal.ZERO) != 0) {
                            ratio = billingAccountAmountWithoutTax.divide(totalBillingAccountAmountWithoutTax, appProvider.getRounding(), RoundingMode.HALF_UP);
                        } else {
                            ratio = BigDecimal.ONE;
                        }
                    } else {
                        ...
                        if (totalBillingAccountAmountWithTax.compareTo(BigDecimal.ZERO) != 0) {
                            ratio = billingAccountAmountWithTax.divide(totalBillingAccountAmountWithTax, appProvider.getRounding(), RoundingMode.HALF_UP);
                        } else {
                            ratio = BigDecimal.ONE;
                        }
                    }
