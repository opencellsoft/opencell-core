...
public class PricePlanMatrixColumnService extends BusinessService<PricePlanMatrixColumn> {
    ...
    public PricePlanMatrixLinesDto populateLinesAndValues(String pricePlanMatrixCode, String data,
        ...
        List<Map.Entry<String, Optional<Attribute>>> columns = new ArrayList<>();
    ...
    private List<PricePlanMatrixLineDto> populateLines(String pricePlanMatrixCode,
        ...
        while (scanner.hasNextLine()) {
            ...
            for (int columnIndex = 0; columnIndex < columns.size(); columnIndex++) {
                ...
                if (columns.get(columnIndex).getKey().contains("|")) {

                    String columnType = columns.get(columnIndex).getKey().split("\\|")[1];
                    String columnCode = columns.get(columnIndex).getKey().split("\\|")[0];
                    boolean isRange = Boolean.valueOf(columns.get(columnIndex).getKey().split("\\|")[2]);
                    switch (columnType) {
                        case "String":
                        ...
                        case "List":
                            ...
                            PricePlanMatrixValueDtoList.add(pricePlanMatrixValueDto);
                            if (
                                    pricePlanMatrixValueDto.getStringValue() != null
                                            && !pricePlanMatrixValueDto.getStringValue().isEmpty()
                                            && columns.get(columnIndex).getValue().get().getAllowedValues() != null
                                            && !columns.get(columnIndex).getValue().get().getAllowedValues().contains(pricePlanMatrixValueDto.getStringValue()))
