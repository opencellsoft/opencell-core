...
public class DDRequestLotOpService extends PersistenceService<DDRequestLotOp> {
    ...
    public List<DDRequestLotOp> getDDRequestOps(DDRequestBuilder ddRequestBuilder,PaymentOrRefundEnum paymentOrRefundEnum) {
        ...
        try {
            Query query = getEntityManager()
                .createQuery("from " + DDRequestLotOp.class.getSimpleName() + " as p  left join fetch p.ddrequestLOT t where p.status=:statusIN and "
                        + "p.ddRequestBuilder=:builderIN and p.paymentOrRefundEnum=:paymentOrRefundEnumIN ")
