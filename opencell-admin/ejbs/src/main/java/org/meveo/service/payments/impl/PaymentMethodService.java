...
public class PaymentMethodService extends PersistenceService<PaymentMethod> {
	...
	/**
	 * Store payment information in payment gateway and return token id in a payment
	 * gateway.
	 * 
	...
	/**
	 * Gets only the hosted checkout url, so to create the PaymentMethod on the
	 * customerAccount, you need to intercept the asynchrone gateway response.
	 *
	...
	public String getHostedCheckoutUrl(HostedCheckoutInput hostedCheckoutInput) throws BusinessException {
		...
		if ((customerAccount.getAddress() != null) && (customerAccount.getAddress().getCountry() != null)
				&& (!StringUtils.isBlank(customerAccount.getAddress().getCountry().getCountryCode()))) {
