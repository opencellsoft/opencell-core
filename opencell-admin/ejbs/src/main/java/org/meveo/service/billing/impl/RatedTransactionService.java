...
public class RatedTransactionService extends PersistenceService<RatedTransaction> {
	...
	public List<RatedTransaction> getRatedTransactionsInvoiced(UserAccount userAccount) {
		...
		return getEntityManager().createNamedQuery("RatedTransaction.listInvoiced", RatedTransaction.class)
	...
	public ConsumptionDTO getConsumption(Subscription subscription, String infoType, Integer billingCycle,
		...
		if (sumarizeConsumption) {
			...
			QueryBuilder qb = new QueryBuilder(
					"select sum(amount1WithTax), sum(usageAmount) from " + RatedTransaction.class.getSimpleName());
	...
	/**
	 * Append invoice aggregates to an invoice. Retrieves all to-invoice Rated
	 * transactions for a given billing account
	 * 
	 * @param billingAccount
	 *            Billing Account
	 * @param invoice
	 *            Invoice to append invoice aggregates to
	 ...
	 *             business exception
	 */
	public void appendInvoiceAgregates(BillingAccount billingAccount, Invoice invoice, Date firstTransactionDate,
		...
		List<RatedTransaction> ratedTransactions = getEntityManager()
				.createNamedQuery("RatedTransaction.listToInvoiceByBillingAccount", RatedTransaction.class)
	...
	/**
	 * Creates Invoice aggregates from given Rated transactions and appends them to
	 * an invoice
	 * 
	 * @param billingAccount
	 *            Billing Account
	 ...
	 * @param isInvoiceAdjustment
	 *            Is this invoice adjustment
	 * @param isVirtual
	 *            Is this a virtual invoice - invoice is not persisted, rated
	 *            transactions are not persisted either
	 * @throws BusinessException
	 *             BusinessException
	 */
	public void appendInvoiceAgregates(BillingAccount billingAccount, Invoice invoice,
			List<RatedTransaction> ratedTransactions, boolean isInvoiceAdjustment, boolean isVirtual)
			throws BusinessException {
		...
		boolean calculateExternalTax = "YES"
				.equalsIgnoreCase((String) appProvider.getCfValue("OPENCELL_ENABLE_TAX_CALCULATION"));
		...
		for (RatedTransaction ratedTransaction : ratedTransactions) {
			...
			scaKey = (ratedTransaction.getUserAccount() != null ? ratedTransaction.getUserAccount().getId() : "") + "_"
					+ (ratedTransaction.getWallet() != null ? ratedTransaction.getWallet().getId() : "") + "_"
					+ invoiceSubCategory.getId() + "_"
			...
			if (scAggregate == null) {
				scAggregate = new SubCategoryInvoiceAgregate(invoiceSubCategory, billingAccount,
						ratedTransaction.getUserAccount(), ratedTransaction.getWallet(), ratedTransaction.getTax(),
						ratedTransaction.getTaxPercent(), invoice, invoiceSubCategory.getAccountingCode());
				...
				if (descTranslated == null) {
					...
					if ((invoiceSubCategory.getDescriptionI18n() != null)
		...
		if ((billingAccount.getDiscountPlanInstances() != null)
				&& !billingAccount.getDiscountPlanInstances().isEmpty()) {
		...
		// Calculate derived aggregate amounts for subcategory aggregate, create
		// category aggregates, discount aggregates and tax aggregates
		...
		for (SubCategoryInvoiceAgregate scAggregate : subCategoryAggregates.values()) {
			...
			InvoiceSubCategory invoiceSubCategory = invoiceSubCategoryService
					.retrieveIfNotManaged(scAggregate.getInvoiceSubCategory());

			// If tax calculation is done on subcategory level, evaluate tax again in case
			// it was changed
			if (calculateTaxOnSubCategoryLevel) {
				Tax tax = null;

				// If there is a taxScript in invoiceSubCategory and script is applicable, use
				// it to compute external taxes
				if (calculateExternalTax && (invoiceSubCategory.getTaxScript() != null)) {
					if (taxScriptService.isApplicable(invoiceSubCategory.getTaxScript().getCode(),
							scAggregate.getUserAccount(), invoice, invoiceSubCategory)) {
						List<Tax> taxes = taxScriptService.computeTaxes(invoiceSubCategory.getTaxScript().getCode(),
								scAggregate.getUserAccount(), invoice, invoiceSubCategory);
						if (!taxes.isEmpty()) {
							tax = taxes.get(0);
						}
				...
				if (tax == null) {
					tax = invoiceSubCategoryCountryService.determineTax(invoiceSubCategory, seller, billingAccount,
							invoice.getInvoiceDate(), false);
				}

				// If tax has changed, need to update RTs with new tax value
				if ((scAggregate.getTaxPercent() == null)
						|| (scAggregate.getTaxPercent().compareTo(tax.getPercent()) != 0)) {
					log.trace("Will update {} rated transactions in subcategory {} with new tax from {} to {}",
							scAggregate.getItemNumber(), scAggregate.getInvoiceSubCategory().getCode(),
							scAggregate.getTaxPercent(), tax.getPercent());
			...
			amounts = NumberUtils.computeDerivedAmounts(scAggregate.getAmountWithoutTax(),
					scAggregate.getAmountWithTax(), scAggregate.getTaxPercent(), isEnterprise, invoiceRounding,
			...
			String caKey = (scAggregate.getUserAccount() != null ? scAggregate.getUserAccount().getId() : "") + "_"
					+ invoiceSubCategory.getInvoiceCategory().getId();

			CategoryInvoiceAgregate cAggregate = categoryAggregates.get(caKey);
			if (cAggregate == null) {
				cAggregate = new CategoryInvoiceAgregate(invoiceSubCategory.getInvoiceCategory(), billingAccount,
						scAggregate.getUserAccount(), invoice);
				...
				if (descTranslated == null) {
					...
					if ((invoiceSubCategory.getInvoiceCategory().getDescriptionI18n() != null) && (invoiceSubCategory
							.getInvoiceCategory().getDescriptionI18n().get(languageCode) != null)) {
			...
			if ((amount != null) && !BigDecimal.ZERO.equals(amount)) {
				...
				for (DiscountPlanItem discountPlanItem : applicableDiscountPlanItems) {
					...
					// Apply discount if matches the category, subcategory, or applies to any
					// category
					if (((discountPlanItem.getInvoiceCategory() == null)
							&& (discountPlanItem.getInvoiceSubCategory() == null))
							|| ((discountPlanItem.getInvoiceSubCategory() != null)
									&& discountPlanItem.getInvoiceSubCategory().getId()
											.equals(scAggregate.getInvoiceSubCategory().getId()))
							|| ((discountPlanItem.getInvoiceCategory() != null)
									&& (discountPlanItem.getInvoiceSubCategory() == null)
									&& discountPlanItem.getInvoiceCategory().getId().equals(
											scAggregate.getInvoiceSubCategory().getInvoiceCategory().getId()))) {
						BigDecimal discountPercent = discountPlanItem.getDiscountValue();

						if (discountPlanItem.getDiscountValueEL() != null) {
							discountPercent = evaluateDiscountPercentExpression(discountPlanItem.getDiscountValueEL(),
									scAggregate.getUserAccount(), scAggregate.getWallet(), invoice, amount);
						...
						if (discountPlanItem.getDiscountPlanItemType().equals(DiscountPlanItemTypeEnum.PERCENTAGE)) {
							discountAmount = amount.multiply(discountPercent.divide(HUNDRED)).negate()
									.setScale(invoiceRounding, invoiceRoundingMode.getRoundingMode());
						} else {
							discountAmount = discountPlanItem.getDiscountValue().negate().setScale(invoiceRounding,
									invoiceRoundingMode.getRoundingMode());
							;
						}
						...
						if (discountAmount.compareTo(BigDecimal.ZERO) < 0) {
							SubCategoryInvoiceAgregate discountAggregate = new SubCategoryInvoiceAgregate(
									scAggregate.getInvoiceSubCategory(), billingAccount, scAggregate.getUserAccount(),
									scAggregate.getWallet(), scAggregate.getTax(), scAggregate.getTaxPercent(), invoice,
									null);
							...
							amounts = NumberUtils.computeDerivedAmounts(discountAmount, discountAmount,
									scAggregate.getTaxPercent(), isEnterprise, invoiceRounding,
									invoiceRoundingMode.getRoundingMode());
				...
				if (calculateTaxOnSubCategoryLevel && (amountCumulativeForTax.compareTo(BigDecimal.ZERO) > 0)) {
					...
					if (taxAggregate == null) {
						taxAggregate = new TaxInvoiceAgregate(billingAccount, scAggregate.getTax(),
								scAggregate.getTaxPercent(), invoice);
						...
						if (descTranslated == null) {
							...
							if ((scAggregate.getTax().getDescriptionI18n() != null)
									&& (scAggregate.getTax().getDescriptionI18n().get(languageCode) != null)) {
		...
		if (calculateTaxOnSubCategoryLevel) {
			for (TaxInvoiceAgregate taxAggregate : taxAggregates.values()) {
				...
				amounts = NumberUtils.computeDerivedAmounts(taxAggregate.getAmountWithoutTax(),
						taxAggregate.getAmountWithTax(), taxAggregate.getTaxPercent(), isEnterprise, invoiceRounding,
						invoiceRoundingMode.getRoundingMode());
		...
		// If tax calculation is not done at subcategory level, then call a global
		// script to do calculation for the whole invoice
		if (!isExonerated && !calculateTaxOnSubCategoryLevel) {
			if ((invoice.getInvoiceType() != null) && (invoice.getInvoiceType().getTaxScript() != null)) {
				taxAggregates = taxScriptService.createTaxAggregates(invoice.getInvoiceType().getTaxScript().getCode(),
						invoice);
		...
		// Calculate invoice total amounts by the sum of tax aggregates or category
		// aggregates minus discount aggregates
		...
		BigDecimal invoicingThreshold = billingAccount.getInvoicingThreshold() == null
				? billingAccount.getBillingCycle().getInvoicingThreshold()
				: billingAccount.getInvoicingThreshold();
		if ((invoicingThreshold != null) && (invoicingThreshold
				.compareTo(isEnterprise ? invoice.getAmountWithoutTax() : invoice.getAmountWithTax()) > 0)) {
	...
	public Boolean isBillingAccountBillable(BillingAccount billingAccount, String orderNumber,
		...
		long count = 0;
		TypedQuery<Long> q = getEntityManager().createNamedQuery("RatedTransaction.countListToInvoiceByOrderNumber",
				Long.class);
		count = q.setParameter("orderNumber", orderNumber).setParameter("firstTransactionDate", firstTransactionDate)
				.setParameter("lastTransactionDate", lastTransactionDate).getSingleResult();
		log.debug("isBillingAccountBillable code={},orderNumber={}) : {}", billingAccount.getCode(), orderNumber,
				count);
		return count > 0 ? true : false;
	}

	/**
	 * This method is only for generating Xml invoice
	 * {@link org.meveo.service.billing.impl.XMLInvoiceCreator #createXMLInvoice(Long, java.io.File, boolean, boolean)
	 * createXMLInvoice}
	 * <p>
	 * If the provider's displayFreeTransacInInvoice of the current invoice is
	 * <tt>false</tt>, RatedTransaction with amount=0 don't show up in the XML.
	 * </p>
	 * 
	 * @param wallet
	 *            wallet instance
	...
	/**
	 * @param invoice
	 *            invoice to get rated transaction.
	...
	/**
	 * @param wallet
	 *            wallet contains the wallet operation.
	 * @param invoice
	 *            invoice to get from
	 * @param invoiceSubCategory
	 *            invoice sub category
	 ...
	 * @throws UnrolledbackBusinessException
	 *             un rolledback business exception
	 */
	 ...
	 *             business exception
	 */
	private BigDecimal evaluateDiscountPercentExpression(String expression, UserAccount userAccount,
	 ...
	 * @return list of rated transaction
	 */
	public List<RatedTransaction> getListByInvoiceAndSubCategory(Invoice invoice,
			InvoiceSubCategory invoiceSubCategory) {
		if ((invoice == null) || (invoiceSubCategory == null)) {
			return null;
		}
		return getEntityManager()
				.createNamedQuery("RatedTransaction.getListByInvoiceAndSubCategory", RatedTransaction.class)
				.setParameter("invoice", invoice).setParameter("invoiceSubCategory", invoiceSubCategory)
				.getResultList();
	}
	 ...
	 *            Wallet operation
	 * @param isVirtual
	 *            Is charge event a virtual operation? If so, no entities should be
	 *            created/updated/persisted in DB
	 * @return Rated transaction
	 * @throws BusinessException
	 *             business exception
	 */
	public RatedTransaction createRatedTransaction(WalletOperation walletOperation, boolean isVirtual)
			throws BusinessException {
	...
	/**
	 * Convert Wallet operations to Rated transactions for a given billable entity
	 * up to a given date
	 * 
	 * @param entity
	 *            entity to bill
	...
	public Long countNotInvoicedRTByBA(BillingAccount billingAccount) {
		try {
			return (Long) getEntityManager().createNamedQuery("RatedTransaction.countNotInvoicedByBA")
					.setParameter("billingAccount", billingAccount).getSingleResult();
		} catch (NoResultException e) {
	...
	public Long countNotInvoicedRTByUA(UserAccount userAccount) {
		try {
			return (Long) getEntityManager().createNamedQuery("RatedTransaction.countNotInvoicedByUA")
	...
	public Long countNotInvoicedRTByCA(CustomerAccount customerAccount) {
		try {
			return (Long) getEntityManager().createNamedQuery("RatedTransaction.countNotInvoicedByCA")
					.setParameter("customerAccount", customerAccount).getSingleResult();
		} catch (NoResultException e) {
	...
	/**
	 * Call RatedTransaction.setStatusToCanceledByRsCodes Named query to cancel just
	 * opened RatedTransaction of all passed RatedTransaction ids.
	 * 
	 * @param rsToCancelIds
	 *            rated transactions to cancel
	 */
	public void cancelRatedTransactions(List<Long> rsToCancelIds) {
		if ((rsToCancelIds.size() > 0) && !rsToCancelIds.isEmpty()) {
			getEntityManager().createNamedQuery("RatedTransaction.setStatusToCanceledByRsCodes")
					.setParameter("rsToCancelCodes", rsToCancelIds).executeUpdate();
		}
	}
	...
	public IBillableEntity updateEntityTotalAmounts(IBillableEntity entity, BillingRun billingRun)
			throws BusinessException {
		...
		if (entity instanceof Subscription) {
			...
			billingAccount = ((Subscription) entity).getUserAccount() != null
					? ((Subscription) entity).getUserAccount().getBillingAccount()
					: null;
		}
		...
		if (invoiceAmount != null) {
			...
			if (invoicingThreshold != null) {
				if (invoicingThreshold.compareTo(invoiceAmount) > 0) {
					log.debug(
							"updateEntityTotalAmounts  invoicingThreshold( stop invoicing)  baCode:{}, amountWithoutTax:{} ,invoicingThreshold:{}",
							entity.getCode(), invoiceAmount, invoicingThreshold);
					return null;
				} else {
					log.debug(
							"updateEntityTotalAmounts  invoicingThreshold(out continue invoicing)  baCode:{}, amountWithoutTax:{} ,invoicingThreshold:{}",
							entity.getCode(), invoiceAmount, invoicingThreshold);
				}
			} else {
			...
			log.debug("{}/{} will be updated with BR amount {}", entity.getClass().getSimpleName(), entity.getId(),
					invoiceAmount);
		}
	...
	/**
	 * Create min amounts rated transactions and set invoiceable amounts to the
	 * billable entity
	 *
	 ...
	 */
	public void calculateAmountsAndCreateMinAmountTransactions(IBillableEntity billableEntity,
		...
		if (billableEntity instanceof Order) {
			...
			// Analyze total amounts per service and subscription and create Rated
			// transactions to reach a minimum amount per service and/or subscription if
			// applicable
			for (Subscription subscription : subscriptionsToProcess) {
				...
				for (ServiceInstance serviceInstance : subscription.getServiceInstances()) {
					...
					if (serviceInstance.getStatus().equals(InstanceStatusEnum.ACTIVE)) {
						Map<String, Amounts> createdAmounts = createMinRTForService(serviceInstance,
								lastTransactionDate, billingAccount, minRatingDate, minAmountTransactions);
						...
						if (createdAmounts != null) {
							for (Entry<String, Amounts> createdAmount : createdAmounts.entrySet()) {
								if (subscriptionLevelExtraAmounts.containsKey(createdAmount.getKey())) {
									subscriptionLevelExtraAmounts.get(createdAmount.getKey())
											.addAmounts(createdAmount.getValue());
								} else {
				...
				// Create RTs to reach min amounts for subscriptions that require it and
				// calculate/update total amounts and amounts per Billing account level
				createMinRTForSubscription(subscription, lastTransactionDate, billingAccount, minRatingDate,
						minAmountTransactions, subscriptionLevelExtraAmounts, totalAmounts, baLevelAmounts);
			}

			if ((billableEntity instanceof BillingAccount)
					&& (totalAmounts.getAmountWithoutTax().compareTo(BigDecimal.ZERO) != 0)) {
				createMinRTForBillingAccount(billingAccount, lastTransactionDate, minRatingDate, minAmountTransactions,
						totalAmounts, baLevelAmounts);
			}
	...
	/**
	 * Create Rated transactions to reach minimum invoiced amount per service.
	 * Updates minAmountTransactions parameter.
	 * 
	 ...
	 *            Billing account
	 * @param minRatingDate
	 *            Date to assign to newly created minimum amount Rated transactions
	 * @param minAmountTransactions
	 *            Newly created minimum amount Rated transactions. ARE UPDATED by
	 *            this method. Rated trancastions created in this method are
	 *            appended.
	 * @return A map of amounts created. With <seller.id>_<invoiceSubCategory.id> as
	 *         a key a and amounts as values
	 * @throws BusinessException
	 *             General business exception
	 */
	private Map<String, Amounts> createMinRTForService(ServiceInstance serviceInstance, Date lastTransactionDate,
			BillingAccount billingAccount, Date minRatingDate, List<RatedTransaction> minAmountTransactions)
			throws BusinessException {
		...
		String minAmountEL = StringUtils.isBlank(serviceInstance.getMinimumAmountEl())
				? serviceInstance.getServiceTemplate().getMinimumAmountEl()
				: serviceInstance.getMinimumAmountEl();
		String minAmountLabelEL = StringUtils.isBlank(serviceInstance.getMinimumLabelEl())
		...
		List<Object[]> amountsList = computeInvoiceableAmountForService(chargeInstances, new Date(0),
				lastTransactionDate);
		...
		while (amountIterator.hasNext()) {
			...
			BigDecimal serviceAmount = appProvider.isEntreprise() ? amountsEntry.getValue().getAmountWithoutTax()
					: amountsEntry.getValue().getAmountWithTax();

			BigDecimal diff = minAmount.subtract(totalServiceAmount);
			BigDecimal ratio = totalServiceAmount.compareTo(serviceAmount) == 0 ? BigDecimal.ONE
					: serviceAmount.divide(totalServiceAmount, 4, RoundingMode.HALF_UP);
			...
			Tax tax = invoiceSubCategoryCountryService.determineTax(invoiceSubCategory, seller, billingAccount,
					minRatingDate, false);

			BigDecimal rtMinAmount = diff.multiply(ratio);

			BigDecimal[] unitAmounts = NumberUtils.computeDerivedAmounts(rtMinAmount, rtMinAmount, tax.getPercent(),
					appProvider.isEntreprise(), BaseEntity.NB_DECIMALS, RoundingMode.HALF_UP);
			BigDecimal[] amounts = NumberUtils.computeDerivedAmounts(rtMinAmount, rtMinAmount, tax.getPercent(),
					appProvider.isEntreprise(), appProvider.getRounding(),
					appProvider.getRoundingMode().getRoundingMode());

			RatedTransaction ratedTransaction = new RatedTransaction(minRatingDate, unitAmounts[0], unitAmounts[1],
					unitAmounts[2], BigDecimal.ONE, amounts[0], amounts[1], amounts[2], RatedTransactionStatusEnum.OPEN,
					null, billingAccount, null, invoiceSubCategory, null, null, null, null, null, null, null, null,
					null, "NO_OFFER", null,
					RatedTransactionMinAmountTypeEnum.RT_MIN_AMOUNT_SE.getCode() + "_" + serviceInstance.getCode(),
					minAmountLabel, null, null, seller, tax, tax.getPercent());

			minAmountTransactions.add(ratedTransaction);

			// Remember newly "created" transaction amounts, as they are not persisted yet
			// to DB
	...
	/**
	 * Create Rated transactions to reach minimum invoiced amount per subscription
	 * and update total amount sum. Updates minAmountTransactions,
	 * baLeveltotalAmounts and baLevelAmounts parameters.
	 * 
	 * @param subscription
	 *            Subscription
	 * @param lastTransactionDate
	 ...
	 *            this method. Rated trancastions created in this method are
	 *            appended.
	 * @param newMinAmountsFromServices
	 *            Additional Rated transaction amounts created in services level
	 *            analysis with <seller.id>_<invoiceSubCategory.id> as key and
	 *            amounts as value.
	 * @param baLeveltotalAmounts
	 *            A sum of amounts irrelevant of seller or invoice subcategory. ARE
	 *            UPDATED by this method. Amounts calculated on service and
	 *            subscription level are appended to the total amounts.
	 * @param baLevelAmounts
	 *            Billing account level amounts. ARE UPDATED by this method. Amounts
	 *            calculated on service and subscription level are appended to the
	 *            total amounts.
	 * @throws BusinessException
	 *             General Business exception
	 */
	private void createMinRTForSubscription(Subscription subscription, Date lastTransactionDate,
			BillingAccount billingAccount, Date minRatingDate, List<RatedTransaction> minAmountTransactions,
			Map<String, Amounts> newMinAmountsFromServices, Amounts baLeveltotalAmounts,
			Map<String, Amounts> baLevelAmounts) throws BusinessException {

		// Initialize subscription level amounts with values of newly created RTS from
		// services
		...
		List<Object[]> amountsList = computeInvoiceableAmountForSubscription(subscription, new Date(0),
				lastTransactionDate);
		...
		// No min amount criteria. DO NOT MOVE this above the total amount
		// calculation/update.
		if (minAmount == null) {
			return;
		}

		BigDecimal totalSubscriptionAmount = appProvider.isEntreprise() ? totalSubscriptionAmountWithoutTax
				: totalSubscriptionAmountWithTax;

		// Subscription level amount exceeds the minimum amount required per
		// subscription
		...
		while (amountIterator.hasNext()) {
			...
			BigDecimal subscriptionAmount = appProvider.isEntreprise() ? amountsEntry.getValue().getAmountWithoutTax()
					: amountsEntry.getValue().getAmountWithTax();
			...
			RatedTransaction ratedTransaction = new RatedTransaction(minRatingDate, unitAmounts[0], unitAmounts[1],
					unitAmounts[2], BigDecimal.ONE, amounts[0], amounts[1], amounts[2], RatedTransactionStatusEnum.OPEN,
					null, billingAccount, null, invoiceSubCategory, null, null, null, null, null, subscription, null,
					null, null, "NO_OFFER", null,
					RatedTransactionMinAmountTypeEnum.RT_MIN_AMOUNT_SU.getCode() + "_" + subscription.getCode(),
					minAmountLabel, null, null, seller, tax, tax.getPercent());
	...
	/**
	 * Create Rated transactions to reach minimum invoiced amount per Billing
	 * account and update total amount sum. Updates minAmountTransactions,
	 * baLeveltotalAmounts and baLevelAmounts parameters.
	 * 
	 * @param billingAccount
	 *            Billing account
	 ...
	 *             General business exception
	 */
	private void createMinRTForBillingAccount(BillingAccount billingAccount, Date lastTransactionDate,
			Date minRatingDate, List<RatedTransaction> minAmountTransactions, Amounts baLeveltotalAmounts,
			Map<String, Amounts> baLevelAmounts) throws BusinessException {

		// Calculate amounts on Billing account level grouped by invoice category. Take
		// into account only those transactions that are not linked to Subscription. The
		// Subscription
		// linked transactions are already present in baLevelAmounts.
		List<Object[]> amountsList = computeInvoiceableAmountForBANotSubscriptionRelated(billingAccount, new Date(0),
				lastTransactionDate);
		...
		BigDecimal totalBaAmount = appProvider.isEntreprise() ? baLeveltotalAmounts.getAmountWithoutTax()
				: baLeveltotalAmounts.getAmountWithTax();

		// Billing account level amount exceeds the minimum amount required per Billing
		// account
		...
		while (amountIterator.hasNext()) {
			...
			BigDecimal baAmount = appProvider.isEntreprise() ? amountsEntry.getValue().getAmountWithoutTax()
					: amountsEntry.getValue().getAmountWithTax();

			BigDecimal diff = minAmount.subtract(totalBaAmount);
			BigDecimal ratio = totalBaAmount.compareTo(baAmount) == 0 ? BigDecimal.ONE
					: baAmount.divide(totalBaAmount, 4, RoundingMode.HALF_UP);
			...
			RatedTransaction ratedTransaction = new RatedTransaction(minRatingDate, unitAmounts[0], unitAmounts[1],
					...
					null, billingAccount, null, invoiceSubCategory, null, null, null, null, null, null, null, null,
					null, "NO_OFFER", null,
					RatedTransactionMinAmountTypeEnum.RT_MIN_AMOUNT_BA.getCode() + "_" + billingAccount.getCode(),
					minAmountLabel, null, null, seller, tax, tax.getPercent());
	...
	/**
	 * Summed rated transaction amounts grouped by invoice subcategory for a given
	 * list of charge instances
	 * 
	 * @param chargeInstances
	 *            Charge instances
	 * @param firstTransactionDate
	 *            First transaction date.
	 * @param lastTransactionDate
	 *            Last transaction date
	 * @return Summed rated transaction amounts as array: sum of amounts without
	 *         tax, sum of amounts with tax, invoice subcategory id
	 */
	@SuppressWarnings("unchecked")
	private List<Object[]> computeInvoiceableAmountForService(List<ChargeInstance> chargeInstances,
			Date firstTransactionDate, Date lastTransactionDate) {
		Query q = getEntityManager().createNamedQuery("RatedTransaction.sumByCharge")
				.setParameter("chargeInstances", chargeInstances)
				.setParameter("firstTransactionDate", firstTransactionDate)
	 ...
	 */
	@SuppressWarnings("unchecked")
	private List<Object[]> computeInvoiceableAmountForSubscription(Subscription subscription, Date firstTransactionDate,
			Date lastTransactionDate) {
		Query q = getEntityManager().createNamedQuery("RatedTransaction.sumBySubscription")
				.setParameter("subscription", subscription).setParameter("firstTransactionDate", firstTransactionDate)
				.setParameter("lastTransactionDate", lastTransactionDate);
		return q.getResultList();
	}
	...
	/**
	 * Summed rated transaction amounts grouped by invoice subcategory for a given
	 * billing account. ONLY those that are not tied to subscription. e.g. product
	 * purchases by User account instead of subscribed products.
	 * 
	 * @param billingAccount
	 *            Billing account
	 * @param firstTransactionDate
	 ...
	 */
	@SuppressWarnings("unchecked")
	private List<Object[]> computeInvoiceableAmountForBANotSubscriptionRelated(BillingAccount billingAccount,
			Date firstTransactionDate, Date lastTransactionDate) {
		Query q = getEntityManager().createNamedQuery("RatedTransaction.sumByBillingAccountNoSubscription")
				.setParameter("billingAccount", billingAccount)
				.setParameter("firstTransactionDate", firstTransactionDate)
				.setParameter("lastTransactionDate", lastTransactionDate);
		return q.getResultList();
	...
	/**
	 * Evaluate double expression. Either ba, subscription or service instance must
	 * be specified.
	 *
	 * @param expression
	 *            EL expression
	 ...
	 *             business exception
	 */
	private BigDecimal evaluateMinAmountExpression(String expression, BillingAccount ba, Subscription subscription,
	 ...
	 *             business exception
	 */
	private String evaluateMinAmountLabelExpression(String expression, BillingAccount ba, Subscription subscription,
			ServiceInstance serviceInstance) throws BusinessException {
		if (StringUtils.isBlank(expression)) {
			return null;
		}
	...
	private Map<Object, Object> constructElContext(String expression, BillingAccount ba, Subscription subscription,
		...
		if (expression.indexOf("ba") >= 0) {
			if (ba == null) {
				ba = subscription != null ? subscription.getUserAccount().getBillingAccount()
						: serviceInstance.getSubscription().getUserAccount().getBillingAccount();
			}
	...
	public Object[] computeOrderInvoiceAmount(Order order, Date firstTransactionDate, Date lastTransactionDate) {
		Query q = getEntityManager().createNamedQuery("RatedTransaction.sumByOrderNumber")
