...
import org.meveo.model.catalog.PricePlanMatrixConfiguration;
...
public class PricePlanSelectionService implements Serializable {
    ...
    public PricePlanMatrix determineDefaultPricePlan(WalletOperation bareWo, Long buyerCountryId, TradingCurrency buyerCurrency) throws NoPricePlanException, InvalidELException {
        ...
        if (matchDbOnly) {
        	return getActivePricePlansByChargeCodeForRatingMatchDB( bareWo, subscriptionAge, buyerCurrency, endDate, endDate, subscriptionAge, endDate, endDate);
    ...
    public PricePlanMatrix getActivePricePlansByChargeCodeForRatingMatchDB( WalletOperation bareWo, Long buyerCountryId, TradingCurrency buyerCurrency, Date startDate, Date endDate, long subscriptionAge, Date subscriptionDate, Date operationDate) {
        ...
        TypedQuery<PricePlanMatrixConfiguration> configQuery = getEntityManager().createNamedQuery("PricePlanMatrixConfiguration.getFirstPricePlanMatrixConfiguration", PricePlanMatrixConfiguration.class).setHint("org.hibernate.cacheable", true);
        configQuery.setMaxResults(1);
        PricePlanMatrixConfiguration configuration = configQuery.getSingleResult();
        
        Map<String, Object> queryParams = new HashMap<>();
        queryParams.put("chargeCode", bareWo.getCode());

        if (configuration.isUseSellerId()) {
            queryBuilder.append(" AND (seller.id = :sellerId OR seller.id IS NULL)");
            queryParams.put("sellerId", bareWo.getSeller().getId());
        }

        if (configuration.isUseOfferId()) {
        ...
        if (configuration.isUseTradingCountryId()) {
        ...
        if (configuration.isUseTradingCurrencyId()) {
