...
public abstract class BaseApi {
	...
	private void populateCustomFields(Map<String, CustomFieldTemplate> customFieldTemplates,
			List<CustomFieldDto> customFieldDtos, ICustomFieldEntity entity, boolean isNewEntity,
		...
		if (customFieldDtos != null && !customFieldDtos.isEmpty()) {
			...
			// Validate fields
			validateAndConvertCustomFields(customFieldTemplates, customFieldDtos, checkCustomFields, isNewEntity,
					entity);
			...
			for (CustomFieldDto cfDto : customFieldDtos) {
				...
				if ((isNewEntity && cft.isHideOnNew()) || (!isNewEntity && !cft.isAllowEdit())
						|| !ValueExpressionWrapper.evaluateToBooleanIgnoreErrors(cft.getApplicableOnEl(), "entity",
								entity)) {
				...
				try {
					...
					if (cft.getFieldType() == CustomFieldTypeEnum.CHILD_ENTITY) {
						...
						for (CustomEntityInstanceDto ceiDto : ((List<CustomEntityInstanceDto>) valueConverted)) {
							...
							childEntityReferences.add(new EntityReferenceWrapper(CustomEntityInstance.class.getName(),
									ceiDto.getCetCode(), ceiDto.getCode()));
						}
					...
					log.error("Failed to set value {} on custom field {} for entity {}", valueConverted,
							cfDto.getCode(), entity, e);
					if (e instanceof MeveoApiException) {
						throw (MeveoApiException) e;
					} else {
						throw new BusinessApiException("Failed to set value " + valueConverted + " on custom field "
								+ cfDto.getCode() + " for entity " + entity);
					}
				}
			}
		}
		...
		for (CustomFieldTemplate cft : customFieldTemplates.values()) {
			...
			if ((isNewEntity && cft.isHideOnNew()) || (!isNewEntity && !cft.isAllowEdit()) || !ValueExpressionWrapper
					.evaluateToBooleanIgnoreErrors(cft.getApplicableOnEl(), "entity", entity)) {
				continue;
			}
			...
			if (!cfisAsMap.containsKey(cft.getCode()) || cfisAsMap.get(cft.getCode()).isEmpty()) {
				...
				// When instance, or multiple instances in case of versioned
				// values, were found
			} else {
				boolean noCfi = true;
	...
	protected void validateAndConvertCustomFields(Map<String, CustomFieldTemplate> customFieldTemplates,
			List<CustomFieldDto> customFieldDtos, boolean checkCustomFields, boolean isNewEntity,
			ICustomFieldEntity entity) throws MeveoApiException {
		...
		for (CustomFieldDto cfDto : customFieldDtos) {
			...
			if (checkCustomFields && cft == null) {
				...
				throw new InvalidParameterException(
						"Custom field template with code " + cfDto.getCode() + " not found.");
			}
				...
				for (Object valueToCheck : valuesToCheck) {
					...
					if (cft.getFieldType() == CustomFieldTypeEnum.STRING) {
						...
						if (cft.getMaxValue() != null && longValue.compareTo(cft.getMaxValue()) > 0) {
							throw new InvalidParameterException("Custom field " + cft.getCode() + " value " + longValue
									+ " is bigger then " + cft.getMaxValue() + ". Allowed value range is from "
							...
							throw new InvalidParameterException("Custom field " + cft.getCode() + " value " + longValue
									+ " is smaller then " + cft.getMinValue() + ". Allowed value range is from "
									+ (cft.getMinValue() == null ? "unspecified" : cft.getMinValue()) + " to "
									+ (cft.getMaxValue() == null ? "unspecified" : cft.getMaxValue()) + ".");
						}
						...
						if (cft.getMaxValue() != null && doubleValue.compareTo(cft.getMaxValue().doubleValue()) > 0) {
							throw new InvalidParameterException(
									"Custom field " + cft.getCode() + " value " + doubleValue + " is bigger then "
											+ cft.getMaxValue() + ". Allowed value range is from "
											+ (cft.getMinValue() == null ? "unspecified" : cft.getMinValue()) + " to "
											+ (cft.getMaxValue() == null ? "unspecified" : cft.getMaxValue()) + ".");
							...
							throw new InvalidParameterException(
									"Custom field " + cft.getCode() + " value " + doubleValue + " is smaller then "
											+ cft.getMinValue() + ". Allowed value range is from "
											+ (cft.getMinValue() == null ? "unspecified" : cft.getMinValue()) + " to "
											+ (cft.getMaxValue() == null ? "unspecified" : cft.getMaxValue()) + ".");
						}
						...
						// requires.
						((CustomEntityInstanceDto) valueToCheck)
								.setCetCode(CustomFieldTemplate.retrieveCetCode(cft.getEntityClazz()));
	...
	/**
	 * Get a value converted from DTO a proper Map, List, EntityWrapper, Date,
	 * Long, Double or String value
	 * 
	 * @return
	 */
	...
	/**
	 * Convert DTO object to an entity. In addition process child DTO object by
	 * creating or updating related entities via calls to API.createOrUpdate().
	 ...
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	protected void convertDtoToEntityWithChildProcessing(Object entityToPopulate, Object dto, boolean partialUpdate)
			throws MeveoApiException {
		...
		for (Field dtoField : FieldUtils.getAllFieldsList(dto.getClass())) {
			...
			try {
				...
				if (entityField == null) {
					log.warn("No match found for field {}.{} in entity {}", dtoClassName, dtoField.getName(),
							entityToPopulate.getClass().getName());
					continue;
				}
				...
				} else if (ReflectionUtils.isDtoOrEntityType(dtoField.getType())
						&& ReflectionUtils.isDtoOrEntityType(entityField.getType())) {
				...
				log.error("Failed to read/convert/populate field value {}.{}. Value {}. Processing will stop.",
						dtoClassName, dtoField.getName(), dtoValue, e);
				throw e;

			} catch (Exception e) {

				log.error("Failed to read/convert/populate field value {}.{}. Value {}", dtoClassName,
						dtoField.getName(), dtoValue, e);
				continue;
			}
		}
	}
	...
	/**
	 * Check if DTO object represents only a reference. In case of reference
	 * only code and provider fields contain values.
	 * 
	 * @param objectToEvaluate
	 *            Dto to evaluate
	 * @return True if only code and provider fields contain values
	 * @throws IllegalAccessException
	 * @throws IllegalArgumentException
	 */
	private boolean isEntityReferenceOnly(Object objectToEvaluate)
			throws IllegalArgumentException, IllegalAccessException {

		for (Field field : FieldUtils.getAllFieldsList(objectToEvaluate.getClass())) {
			if (Modifier.isStatic(field.getModifiers()) || field.getType().isPrimitive()
					|| field.getName().equals("code") || field.getName().equals("provider")) {
				continue;
			}
	...
	/**
	 * Get a corresponding API service for a given DTO object. Find API service
	 * class first trying with item's classname and then with its super class (a
	 * simplified version instead of trying various classsuper classes)
	 * 
	 * @param dto
	 *            DTO object
	 ...
	 * with its super class (a simplified version instead of trying various
	 * class superclasses)
	 * 
	 * @param classname
	...
	protected ApiService getApiService(Class entityClass, boolean throwException) {
		...
		if (apiService == null && throwException) {
			throw new RuntimeException(
					"Failed to find implementation of API service for class " + entityClass.getName());
		}
	...
	protected PersistenceService getPersistenceService(BaseDto dto, boolean throwException) throws MeveoApiException {
		...
		PersistenceService persistenceService = (PersistenceService) EjbUtils
				.getServiceInterface(entityClassName + "Service");
		...
		if (persistenceService == null && throwException) {
			throw new MeveoApiException(
					"Failed to find implementation of persistence service for class " + dto.getClass());
		}
	...
	/**
	 * Find Persistence service class first trying with JPA entity's classname
	 * and then with its super class (a simplified version instead of trying
	 * various class superclasses)
	 * 
	 * @param entityClass
	 *            JPA Entity class
	 * @param throwException
	 ...
	 */
	@SuppressWarnings("rawtypes")
	protected PersistenceService getPersistenceService(Class entityClass, boolean throwException)
			throws MeveoApiException {

		PersistenceService persistenceService = (PersistenceService) EjbUtils
				.getServiceInterface(entityClass.getSimpleName() + "Service");
		if (persistenceService == null) {
			persistenceService = (PersistenceService) EjbUtils
					.getServiceInterface(entityClass.getSuperclass().getSimpleName() + "Service");
		}
		if (persistenceService == null && throwException) {
			throw new MeveoApiException(
					"Failed to find implementation of persistence service for class " + entityClass.getName());
		}

		return persistenceService;
	}

	protected void saveImage(IEntity entity, String imagePath, String imageData)
			throws IOException, MeveoApiException {
