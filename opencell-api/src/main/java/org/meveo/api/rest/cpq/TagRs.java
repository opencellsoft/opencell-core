...
public interface TagRs extends IBaseRs {
	...
	@DELETE
	@Path("/{codeTagType}")
    @Operation(summary = "This endpoint allows to  deleting an existing Tag type",
    description ="Deleting an existing Tag type with its code",
    tags = { "Tag" },
    responses = {
            @ApiResponse(responseCode="200", description = "The Tag type successfully deleted",
            		content = @Content(schema = @Schema(implementation = ActionStatusEnum.class))),
            @ApiResponse(responseCode = "404", description = "No Tag type found for the codeTag parameter", 
    		content = @Content(schema = @Schema(implementation = BusinessException.class))),
            @ApiResponse(responseCode = "400", description = "Impossible to delete a Tag type, because it is attached to a Tag", 
    		content = @Content(schema = @Schema(implementation = BusinessException.class)))
    })
	Response deleteTagType(@Parameter(description = "contain the code of tag te be deleted by its code", required = true) @PathParam("codeTagType") String codeTag);
