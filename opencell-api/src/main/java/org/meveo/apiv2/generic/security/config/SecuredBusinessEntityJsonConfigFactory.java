...
public class SecuredBusinessEntityJsonConfigFactory implements SecuredBusinessEntityConfigFactory {
    ...
    /**
     * Map to store loaded SE configs from json file and stored with key using format entityName.apiMethod
     */
    private Map<String, SecuredBusinessEntityConfig> configMap = new HashMap<>();

    /**
     * Get the Json based config corresponding to the entity name and API method name retrieved form method invocation context
     * 
    ...
    public SecuredBusinessEntityConfig get(InvocationContext context) {
        ...
        return get(entityClass, methodName);
    }
    ...
    /**
     * Fill the empty config parameters with the default values as defined for this factory
     * 
    ...
    private void laodConfigs() {
        ...
        try {
            InputStream configInputStream = this.getClass().getClassLoader().getResourceAsStream("secured-entities-config.json");
            ...
            SecuredBusinessEntityJsonConfig[] sbeConfigsArray = mapper.readValue(configInputStream, SecuredBusinessEntityJsonConfig[].class);
            ...
            throw new RuntimeException("Can't load secured business entities from file " + "secured-entities-config.json", e);
        }
    }
}
