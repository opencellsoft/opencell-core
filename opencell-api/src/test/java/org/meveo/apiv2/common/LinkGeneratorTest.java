...
public class LinkGeneratorTest {
    ...
    public void testCreateSelfLinkToResource(){
        ...
        Link link = linkGenerator
                .withPostAction()
                .withGetAction().withDeleteAction()
                .withPatchAction()
                .withId(123L)
                .build();
    ...
    public void testResolveNextAndPreviousLinkToResource(){
        ...
        assertEquals(links.get(0).getUri(), UriBuilder.fromResource(ProductResource.class).queryParam("offset",0)
                .queryParam("limit",10).build());
        assertEquals(links.get(1).getUri(), UriBuilder.fromResource(ProductResource.class).queryParam("offset",15)
                .queryParam("limit",10).build());
    ...
    public void testResolveOnlyNextLinkToResource(){
        ...
        assertEquals(links.get(0).getUri(), UriBuilder.fromResource(ProductResource.class).queryParam("offset",10)
                .queryParam("limit",10).build());
    ...
    public void testResolveOnlyPreviousLinkToResource(){
        ...
        assertEquals(links.size(),1);
        assertEquals(links.get(0).getRel(),"previous");
        assertEquals(links.get(0).getUri(), UriBuilder.fromResource(ProductResource.class).queryParam("offset",80)
                .queryParam("limit",10).build());
    }
}
