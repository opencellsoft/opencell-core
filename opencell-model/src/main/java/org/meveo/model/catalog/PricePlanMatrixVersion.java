...
@NamedQueries({
        ...
        @NamedQuery(name = "PricePlanMatrixVersion.getPublishedVersionValideForDate", query = "select distinct v from PricePlanMatrixVersion v left join v.pricePlanMatrix m  where m.id=:pricePlanMatrixId and v.status=org.meveo.model.cpq.enums.VersionStatusEnum.PUBLISHED and (v.validity.from is null or v.validity.from<=:operationDate) and (v.validity.to is null or v.validity.to>:operationDate)"),
