...
@NamedQueries({
        ...
        @NamedQuery(name = "PricePlanMatrixVersion.findToDateAfterDateAndVersion", query = "select p from PricePlanMatrixVersion p where p.status='PUBLISHED' and p.pricePlanMatrix=:pricePlanMatrix and p.validity.to >= :date and p.currentVersion < :version order by p.currentVersion"),
