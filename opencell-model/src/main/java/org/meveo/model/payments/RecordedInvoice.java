...
@NamedQueries({
        @NamedQuery(name = "RecordedInvoice.listRecordedInvoiceIdsToPay", query = "Select ri.id from RecordedInvoice as ri,PaymentMethod as pm  where ri.matchingStatus ='O' and "
                + " ri.customerAccount.excludedFromPayment = false and ri.customerAccount.id = pm.customerAccount.id and pm.paymentType =:payMethod  and pm.preferred is true and ri.unMatchingAmount <> 0"),
