...
@NamedQueries({
    ...
    @NamedQuery(name = "SecurityDeposit.securityDepositsToRefundIds", query = "SELECT s.id FROM SecurityDeposit s WHERE (s.validityDate >=:sysDate or s.validityPeriod is not null) and (s.status = 'LOCKED' or s.status = 'UNLOCKED' or s.status = 'HOLD'")
})
