...
@NamedQueries({
		...
		@NamedQuery(name = "InvoiceLine.cancelByInvoiceIds", query = "update InvoiceLine il set il.status='CANCELED', il.auditable.updated = :now, il.billingRun= null, il.invoice=null, il.invoiceAggregateF=null WHERE il.invoice.id IN (:invoicesIds) and orderOffer is null"),
		...
		@NamedQuery(name = "InvoiceLine.passInvoiceLinesToBilled", query = "UPDATE InvoiceLine il set il.status=org.meveo.model.billing.InvoiceLineStatusEnum.BILLED, il.auditable.updated = :now where il.invoice.id= :invoiceId")
