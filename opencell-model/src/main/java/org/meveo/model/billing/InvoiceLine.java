...
@NamedQueries({
		...
		@NamedQuery(name = "InvoiceLine.listToInvoiceByOrderNumberAndBR", query = "FROM InvoiceLine il where il.status='OPEN' AND il.orderNumber=:orderNumber AND (il.billingRun is null OR il.billingRun.id=:billingRunId) order by il.billingAccount.id "),
		@NamedQuery(name = "InvoiceLine.listToInvoiceBySubscriptionAndBR", query = "FROM InvoiceLine il where il.subscription.id=:subscriptionId AND il.status='OPEN' AND (il.billingRun is null OR il.billingRun.id=:billingRunId)"),
		@NamedQuery(name = "InvoiceLine.listToInvoiceByBillingAccountAndBR", query = "FROM InvoiceLine il where il.billingAccount.id=:billingAccountId AND il.status='OPEN'  AND (il.billingRun is null OR il.billingRun.id=:billingRunId) "),
		...
		@NamedQuery(name = "InvoiceLine.listToInvoiceByCommercialOrderAndBR", query = "FROM InvoiceLine il where il.commercialOrder.id=:commercialOrderId AND il.status='OPEN' AND (il.billingRun is null OR il.billingRun.id=:billingRunId)"),
		...
		@NamedQuery(name = "InvoiceLine.listToInvoiceByQuoteAndBR", query = "FROM InvoiceLine il where il.quote.id=:quoteId AND il.status='OPEN' AND (il.billingRun is null OR il.billingRun.id=:billingRunId)"),
