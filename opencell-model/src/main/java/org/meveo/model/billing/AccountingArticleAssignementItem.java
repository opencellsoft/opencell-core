...
public class AccountingArticleAssignementItem {
	private Long chargeTemplateId;
	private Long offerTemplateId;
	private Long serviceInstanceId;
	private List<Long> chargeInstancesIDs = new ArrayList<>();

	public AccountingArticleAssignementItem(Long chargeTemplateId, Long offerTemplateId, Long serviceInstanceId,
			String chargeInstances) {
		this.chargeTemplateId = chargeTemplateId;
		this.offerTemplateId = offerTemplateId;
		this.serviceInstanceId = serviceInstanceId;
		this.chargeInstancesIDs = StringUtils.isBlank(chargeInstances) ? null : Pattern.compile(",").splitAsStream(chargeInstances).mapToLong(Long::parseLong).boxed()
				.collect(Collectors.toList());
	}

	public AccountingArticleAssignementItem(Object[] fields) {
		int i = 0;
