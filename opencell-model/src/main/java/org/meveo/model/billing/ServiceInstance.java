...
@NamedQueries({
        ...
        @NamedQuery(name = "ServiceInstance.getServicesWithMinAmountBySubscription", query = "select s from ServiceInstance s where (s.minimumAmountEl is not null or s.serviceTemplate.minimumAmountEl is not null) AND s.status = org.meveo.model.billing.InstanceStatusEnum.ACTIVE AND s.subscription=:subscription"),
        @NamedQuery(name = "ServiceInstance.getServicesWithMinAmountByBA", query = "select s from ServiceInstance s where (s.minimumAmountEl is not null or s.serviceTemplate.minimumAmountEl is not null) AND s.status = org.meveo.model.billing.InstanceStatusEnum.ACTIVE AND s.subscription.userAccount.billingAccount=:billingAccount")
})
